...
var reportsRouter = !window.commonRouter ? (function($, window) {
	...
	var init = function() {
		window.onpopstate = function(event) {
			if (typeof event.state === 'object' && event.state != null) {
				var history = window.JSON.parse(sessionStorage.getItem('history'));
				$(event.state.container).html(history[event.state.url]).unblock();
	...
	var setNewState = function(newState) {
		...
		history[newState.url] = newState.html;
		try{
