...
if ($actionTarget == 'downloadExportedReport') {
	...
	if ($getContent) {
		...
		if ($fileType == 'app' && $openPopup) {
			...
			$extraParams = array(
				...
				'csvData' => array(
					'reportName' => $unserializedReport['name'],
	...
	if (!empty($currentQueryData['module'])) {
		...
		$externalDataBaseQueryParams = asol_ReportsGenerationFunctions::manageExternalDatabaseQueries($alternativeDb, $currentQueryData['module']);
		...
		$currentFields = json_decode($currentQueryData['fields'], true);
		$currentFilters = json_decode($currentQueryData['filters'], true);
		
		$avoidTrim = $currentFilters['config']['avoidTrim'];
		$avoidEmptyFilters= $currentFilters['config']['avoidEmptyFilters'];
		$avoidAutocomplete = $currentFilters['config']['avoidAutocomplete'];
		...
		$extraParams = array(
			...
			'currentConfig' => $currentFields['tables'][0]['config'],
			...
			'avoidTrim' => $avoidTrim,
			'avoidEmptyFilters' => $avoidEmptyFilters,
			'avoidAutocomplete' => $avoidAutocomplete,
		);
