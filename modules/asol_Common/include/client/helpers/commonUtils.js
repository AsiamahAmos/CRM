function setCodeMirrorEditor(textAreaId, editorMode) {
	...
	text/css
	*/
	if (typeof window['codeMirror_'+textAreaId] === "undefined" ||  !( $('#'+textAreaId).siblings('.CodeMirror')[0] && $._data( $('#'+textAreaId).siblings('.CodeMirror')[0], "events" )) ) {
		
		if( $('#'+textAreaId).siblings('.CodeMirror')[0] && !$._data( $('#'+textAreaId).siblings('.CodeMirror')[0], "events" ) ){
			$('#'+textAreaId).siblings('.CodeMirror').remove(); //Delete death CodeMirror instances
		}
...
var getElementNameById = function(module, id) {  //Asynchronous method return name of Id
	
	return {
		done : function( functionParameter ){
			
			$.ajax({ 
				dataType: "json",
				url: 'index.php?entryPoint=commonAjaxActions&actionTarget=getBasicModuleSearch&commonDB=-1&commonModule='+module+'&commonFields=%5B%7B%22field%22%3A%22name%22%7D%5D&&commonFilters=%5B%7B%22field%22%3A%22id%22%2C%22value%22%3A%22'+id+'%22%7D%5D',
			}).done(function(data){
				functionParameter( data.rows[0].name );
			});
...
//Check if code color has '#' at the first position
var checkColorCodeFormat = function (colors) {
	if (colors != undefined) {
		colors.forEach(function(item, loop) {
			if (item[0] != "#") {
				colors[loop] = "#" + colors[loop];
			}
		});
	}
