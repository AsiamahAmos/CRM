...
class asol_ReportsDispatcher {
	
	private static function applysDistpacher() {
		
		global $sugar_config;
		
	 	return isset($sugar_config['asolReportsMaxExecutionTime']) && ($sugar_config['asolReportsMaxExecutionTime'] > 0) && isset($_REQUEST["reportRequestId"]) && isset($_REQUEST["initRequestDateTimeStamp"]);
	...
	public static function setLastQuery($query) {
		...
		if (self::applysDistpacher()) {
			$db->query("UPDATE asol_reports_dispatcher SET last_sql = '".mysqli_real_escape_string($query)."' WHERE id='".$_REQUEST["reportRequestId"]."' LIMIT 1");
		}
...
class asol_Reports extends Basic {
	...
	static public $elapsed_time = 0.00;
	...
	*/
	...
	static public function getSelectionResults($query, $columnAsKey = null, $useAlternativeDb = true, $notCrmExternalDb = false, $overrideConnection = null, $detailColumn = null, $formatInfo = null, $avoidLogging = false) {
		...
		asol_ReportsDispatcher::setLastQuery($query);
		...
		if (((!isset($sugar_config["asolReportsDbAddress"])) || (!$useAlternativeDb)) && ($notCrmExternalDb === false)) {
			...
			while ($queryRow = $db->fetchByAssoc($queryResults)) {
				if (isset($detailColumn)) {
					$theGroup = asol_ControllerQuery::formatDateSpecialsGroup($formatInfo['reportId'], $formatInfo['dashletId'], $queryRow['asol_grouping_field'], $detailColumn, $formatInfo['userDateFormat'], $formatInfo['userTZ'], $formatInfo['gmtDates']);
		...
		self::$elapsed_time += (microtime(true) - $initialTimestamp);
	...
	static public function getEnumLabels($data_source, $field, $operator, $reference) {

		global $app_list_strings, $current_language;
		
		$enumLabels = array();
		
		if ($data_source['type'] == '0') { //DB
			...
			if ($operator == 'function') {
				$enumLabels = $reference();
			} else if ($operator == 'options') {
				$enumLabels = $app_list_strings[$reference];
			}

		} else if ($data_source['type'] == '1') { //Api
			
			if ($operator == 'function') {
				//***AlineaSol Premium***//
				$extraParams = array('data_source' => $data_source, 'field' => $field);
				$apiReportResult = asol_ReportsUtils::managePremiumFeature("externalApiReports", "reportFunctions.php", "getExternalApiEnumValues", $extraParams);
				$enumLabels = ($apiReportResult !== false ? $apiReportResult : array());
				//***AlineaSol Premium***//
			}
	...
	static public function getEnumValues($data_source, $field, $operator, $reference) {

		return array_keys(self::getEnumLabels($data_source, $field, $operator, $reference));
