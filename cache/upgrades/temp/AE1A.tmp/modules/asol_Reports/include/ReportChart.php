...
class asol_ReportsCharts {
	...
	static public function fetchDataNormalized($subTotalsCharts, $xAxesAlias, $function) {
		...
		foreach ($newData as $position => $data) {
			foreach ($data as $key => $value ) {
				if (!in_array($key, $xAxesAlias)) {
					$data[$key] = asol_ControllerQuery::getManuallyTotalCharts($function, explode(';', $data[$key]));
				}
			}
	...
	static public function getDataForChartsGeneration(& $chartInfo, & $chartConfig, $fieldValues, $subTotalsCharts, $subGroupsExport, $massiveData, $isDetailReport, $isGroupedReport, $hasFunctionField, $groups, $groupExport, $userDateFormat) {
		...
		foreach ($chartInfo as $chartKey => $currentChartInfo) {
			...
			$currentSubTotalsCharts = $subTotalsCharts;
			
			//Check if x-Axis has not selected all of the values, if so, keep them by default
			if ((count($currentChartInfo['xAxis']) != count($currentGroups)) && (count($currentChartInfo['xAxis']) != 0) && (!$isDetailReport)) {

				$xAxesAlias = self::getFieldAliasFromXAxis($currentChartInfo['xAxis'], $fieldValues['tables'][0]['data']);
				$currentSubTotalsCharts = self::normalizeSubTotalCharts($currentSubTotalsCharts, $xAxesAlias, $currentGroups);
				$currentSubTotalsCharts = self::fetchDataNormalized($currentSubTotalsCharts, $xAxesAlias, $currentChartInfo['function']);

				$currentGroups = self::normalizeGroupsWithXAxis($xAxesAlias, $currentGroups);
			...
			if (strtolower($currentChartInfo['display']) == 'yes') {
				...
				if ($isDetailReport) {
					...
					if ($hasFunctionField) {
						...
						foreach($currentSubTotalsCharts as $groupKey=>$totalValues) {
							...
							if (in_array($currentChartInfo['type'], array('pie', 'donut', 'bar', 'funnel'))) {
								...
								$subGroupsChart[$currentChartLabel][$counter] = $groupKey;
								...
								if ($isGroupedReport) {
									...
									foreach ($subGroupsExport[$groupKey] as $groupExport) {
										...
										$chartValues[$currentChartLabel]['mainChart'][$counter]['values'][] = array (
											'group' => asol_ControllerQuery::getGroupingChartValue($currentGroups, $groupExport, $userDateFormat),
							...
							} else if( $currentChartInfo['type'] = 'map' ) {
								
								$counter = count($chartValues[$currentChartLabel]['mainChart']);
								$chartValues[$currentChartLabel]['mainChart'][$counter]= $currentValue;
					...
					if (in_array($currentChartInfo['type'], array('pie', 'donut', 'bar', 'funnel', 'stack', 'horizontal', 'line', 'scatter', 'area', 'bubble'))) {
						...
						foreach ($currentSubTotalsCharts as $key=>$values) {
							$subGroupsChart[$currentChartLabel][$key] = asol_ControllerQuery::getGroupingChartValue($currentGroups, $values, $userDateFormat);
				...
				if (isset($chartInfo[$chartKey])) {
					...
					if ($hasSubCharts) {
						...
						$extraParamsValues = array(
							...
							'subTotalsCharts' => $currentSubTotalsCharts,
							'subGroupsExport' => $subGroupsExport,
							'currentChartLabel' => $currentChartLabel,
							'groupBySequence' => $currentGroups,
	...
	static public function getChartFilesWithExtraData($chartsEngine, $isStackedChart, $massiveData, & $chartInfo, $chartConfigs, $chartTemplates, $predefinedColorTemplates, $chartYAxisLabels, $chartValues, $subGroupsChart, $reportId, $dashletId, $reportModule, $chartsHttpQueryUrls, $isDetailedReport, $isGroupedReport, $isStoredReport) {
		...
		foreach ($chartValues as $currentChartValues) {
			...
			if ( $currentChartInfo['type'] == 'map' && $chartsEngine == 'nvd3') { 
							
				$urlChart[] = self::generateNvd3NormalChartData($reportId, $dashletId, $currentChartTypes, $chartYAxisLabels[$chartIndex], $currentChartYAxisSides, $currentChartConfigs, $currentChartTemplates, $predefinedColorTemplates, count($urlChart), $reportModule, $currentChartInfo['label'], $currentSubGroupsChart, $currentChartValues, count($currentSubGroupsChart), (empty($chartsHttpQueryUrls)) ? null : $chartsHttpQueryUrls[count($urlChart)], $currentChartInfo['function'], $isStoredReport, $currentChartInfo['halfChart']);
			
				$chartInfo[$chartIndex]['type'] = 'MapChart';
				$chartInfo[$chartIndex]['subgroups'] = count($currentSubGroupsChart);
				$chartSubGroupsValues[] = count($currentSubGroupsChart);
	...
	static public function getChartEngineLibraries($chartEngine, $isDashlet, $isPreview, $callback = null) {
		...
		$reportsLibrariesPath = 'modules/asol_Reports/include/client/libraries';
		...
		switch ($chartEngine) {
			...
			case "html5":
				...
				$chartEngineLibraries[] = ($isDashlet) ? "JS;0;modules/asol_Reports/include/js/sugarCharts.min.js" : "JS;0;include/SugarCharts/Jit/js/sugarCharts.js";
				break;
			...
			case "nvd3":
				...
				$chartEngineLibraries[] = "JS;0;modules/asol_Common/include/client/libraries/chromajs/chroma.js";
				break;
	...
	static public function getCrmChartHtml($reportId, $chartEngine, $urlChart, $chartInfo, $chartsReferences, $current_language, $theme, $isStoredReport, $isDashlet, $dashletId, $chartLayoutConfig) {
		...
		if ($chartEngine == "flash") { //FLASH
			...
			foreach ($urlChart as $key=>$value) {
				...
				if ($returnData) {
					...
					if($chartInfo[$key]['halfChart'] != 'yes' && empty($chartLayoutConfig)) {
						$h = 400;
					} else if( !empty($chartLayoutConfig)) {
	...
	static public function generateNvd3NormalChartData($reportId, $dashletId, $chartTypes, $chartYAxisLabels, $chartYAxisSides, $chartConfigs, $chartTemplates, $predefinedColorTemplates, $prefixChart, $reportModule, $chartTitle, $chartLegends, $chartValues, $numGroups, $overridedName = null, $chartFunction = '0', $isStoredReport = false, $isHalfChart = 'no'){
		...
		if (!$hasSubCharts) {
			...
			if ($chartType != 'parallel') {
				$nvd3ChartDataJs = self::getNvd3NormalDataJs($chartTypes[0], $chartYAxisLabels[0], $indexKey, $nvd3ColorPalete, $chartLegends, $chartValues, $maxChartValue, $chartConfigs);
			} else {
				...
				$nvd3ChartDataJs = self::getNvd3NormalDataJs($chartType, $chartYAxisLabels[0], $indexKey, $nvd3ColorPalete, $chartLegends, $chartValues, $maxChartValue, $chartConfigs);
			}
		...
		switch ($chartType){
			...
			case 'map':
				$jsData = "try {".
						"getMap(".json_encode(array('id' => $reportId, 'config' => $chartConfigs, 'values' => array_combine($chartLegends, $chartValues['mainChart']), 'valName' => $chartYAxisLabels, 'format' => $chartD3Format)).");".
...
// 			if (!isset($mainConfig['mode'])){
// 				$colorMainPalete = array_map(create_function('$color', 'return "#".$color;'), $colorMainPalete);
// 			} else if ($mainConfig['mode'] == "auto") {
// 				$colorMainPalete = $mainConfig['autoColors'];
// 			}
	...
	static private function getNvd3NormalDataJs($chartType, $chartYAxisLabel, $indexKey, $colorPalete, $chartLegends, $chartValues, $maxValue, $chartConfigs) {
		$mainChart = $chartValues ['mainChart'];
		$hasSubCharts = (! empty ( $chartValues ['subCharts'] ));
		...
		switch ($chartType) {
			...
			case "donut" :
				...
				$jsData = 'var chartObject = getNvd3DataJs(' . json_encode ( $chromaParams ) . ', null);';
				$jsData .= 'var data_' . $indexKey . ' = chartObject;';
				$jsData .= 'var asolColorPalete_' . $indexKey . ' = chartObject.map( a => a.color);';
			...
			case "bar" :
				...
				$jsData = 'var chartObject = getNvd3DataJs(' . json_encode ( $chromaParams ) . ', null);';
				$jsData .= 'var data_' . $indexKey . ' = [];' . 'data_' . $indexKey . '.push(chartObject);';
				$jsData .= 'var asolColorPalete_' . $indexKey . ' = chartObject.values.map( a => a.color);';
			...
			case "bubble" :
				...
				$chromaStackedParams = array (
						'dataGroups' => $dataGroups,
						'dataGroupsZ' => $dataGroupsZ 
				);
				
				$jsData = 'var chartObject = getNvd3DataJs(' . json_encode ( $chromaParams ) . ', ' . json_encode ( $chromaStackedParams ) . ');';
				$jsData .= 'var data_' . $indexKey . ' = [];' . 'data_' . $indexKey . '.push(chartObject);';
				$jsData .= 'var asolColorPalete_' . $indexKey . ' = chartObject.values.map( a => a.color);';
				...
				break;
			case "parallel" :
				$jsData = 'var chartObject = getNvd3DataJs(' . json_encode ( $chromaParams ) . ');';
				$jsData .= 'var data_' . $indexKey . ' = chartObject[0];';
				$jsData .= 'var asolColorPalete_' . $indexKey . ' = chartObject[0].map( a => a.color);';
