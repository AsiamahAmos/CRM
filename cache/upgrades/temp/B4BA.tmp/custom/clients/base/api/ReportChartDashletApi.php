    ...
    public function GetReportData($api, $args) {
		...
		if ($displayReportDashlet) {
			...
			if (!empty($reportId)) {
				...
				'<script type="text/javascript">'.asol_ReportsGenerationFunctions::getLoadCurrentDashletScriptFunction($reportId, true, $dashletId, false, null, null, $current_user->id).'</script>'.
				'<div class="table table-striped dataTable">
					'.asol_ControllerReports::displayReport($reportId, null, '', '', '', '', true, $dashletId, false, false, true, false, null, null, null, true, null).'
				</div>';
