...
class asol_CheckConfigurationDefsFunctions {
	...
	static public $errorTips = array(
		...
		'asolReportsExternalApiConnections' => 'Failed to Connect to one of the External Apis with the given parameters or the configured value is not an <b>Array</b>.',
		...
		'asolReportsCurlRequestTimeout' => 'The configured value must be an Integer (miliseconds)',
	...
	static public function checkConfiguration($configParam, $noLongerAvailable = false) {
		...
		if (isset($sugar_config[$configParam])) {
			...
			switch ($configParam) {
				...
				case "asolReportsCurlRequestTimeout":
					if ((!is_numeric($sugar_config[$configParam])) || ($sugar_config[$configParam] < 0)) {
						$hasError = true;
						$posibleResolution = "· \$sugar_config[\&quot;".$configParam."\&quot;] = 500;";
					}
					break;
				...
				case "asolReportsExternalApiConnections":
					if (!self::checkApiConnections()) {
						$hasError = true;
						$posibleResolution = "· Check your SugarCRM External API URL and Credentials.";
					}
					break;
	...
	static private function getImgInfoIcon($configParam, $overrideConfigParam, $posibleValues, $httpRequestParam = false, $isError = true) {
		...
		if ($isError) {
			...
			return "<i id='".$configParam."_Info' style='display: ".($httpRequestParam ? 'none' : 'inline')."; font-size: 1em;' border='0' class='icn-info-circled inlineHelpTip' alt='Info' src='modules/asol_Reports/include/images/asol_reports_help_inline.gif' onClick='return SUGAR.util.showHelpTips(this, &quot;<div class=\\\"detail view  detail508\\\"><table class=\\\"list view\\\"><tr><td width=\\\"20%\\\"><b>Error:</b></td><td width=\\\"80%\\\">".$errorTip."</td></tr><tr><td width=\\\"20%\\\"><b>Posible Solutions:</b></td><td width=\\\"80%\\\">".$posibleValues."</td></tr></table></div>&quot;, &quot;&quot;, &quot;&quot;);'></i>";
		
		} else {
			
			return "<i id='".$configParam."_Info' style='display: ".($httpRequestParam ? 'none' : 'inline')."; font-size: 1em;' border='0' class='icn-info-circled inlineHelpTip' alt='Info' src='modules/asol_Reports/include/images/asol_reports_help_inline.gif' onClick='return SUGAR.util.showHelpTips(this, &quot;<div class=\\\"detail view  detail508\\\"><table class=\\\"list view\\\"><tr><td width=\\\"20%\\\"><b>Posible Values:</b></td><td width=\\\"80%\\\">".$posibleValues."</td></tr></table></div>&quot;, &quot;&quot;, &quot;&quot;);'></i>";
	...
	static private function checkApiConnections() {
		
		global $sugar_config;
		
		$returnedVal = true;
		
		if (is_array($sugar_config['asolReportsExternalApiConnections'])) {
			
			foreach ($sugar_config['asolReportsExternalApiConnections'] as $currentApi) {
				
				require_once $currentApi['asolReportsApiClass'];
				$apiInstance = new $currentApi['asolReportsApiName'];
				
				if ($apiInstance::checkConnectivity($currentApi) === false) {
					$returnedVal = false;
					break;
				}
	...
	static private function checkExternalTls() {
		...
		if (is_array($sugar_config['asolReportsTlConfigurations'])) {
			...
			foreach ($sugar_config['asolReportsTlConfigurations'] as $extTl) {
				...
				} else if ((!is_string($extTl["label"])) || (!is_string($extTl["data"]["mode"]))) {
	...
	static public function checkFileAccess($htmlCheckTagName, $htmlCheckTagId, $fileName) {
		...
		asolJquery.ajax({
	        ...
	        cache: false,
			success: function (data) {
				asolJquery("#'.$htmlCheckTagId.'_Result").html("[200: OK]");
				asolJquery("#'.$htmlCheckTagId.'_Result").css("color", "green");
			},
			error: function (data, textStatus, errorThrown) {
				if (data.status === 200) {
					...
					asolJquery("#'.$htmlCheckTagId.'_Result").html("[" + data.status + ": " + errorThrown + "]");
					asolJquery("#'.$htmlCheckTagId.'_Result").css("color", "red");
					asolJquery("#'.$htmlCheckTagId.'_Info").css("display", "inline");
				}
			}
	    });
		</script>';
