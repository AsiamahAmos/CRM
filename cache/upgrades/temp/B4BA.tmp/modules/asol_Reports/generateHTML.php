...
if ($htmlTarget == 'reportModuleTables') {
	...
	$data_source = json_decode(rawurldecode(html_entity_decode($_REQUEST['data_source'])), true);
	
	if ($data_source['type'] == '0') {
	
		$selectedDb = $data_source['value']['database'];
		$selectedTable = $data_source['value']['module'];
		$auditedReport = $data_source['value']['audit'];
		...
		//***AlineaSol Premium***//
		//***********************//
		$extraParams = array('isSubQuery' => $isSubQuery, 'data_source' => $data_source);
		$externalApiTables = asol_ReportsUtils::managePremiumFeature("externalApisReports", "reportFunctions.php", "getExternalApiTables", $extraParams);
		$returnedHtml = ($externalApiTables !== false ? $externalApiTables : null);
		//***********************//
		//***AlineaSol Premium***//
		//***********************//
	...
	if ($data_source['type'] == '0') {
		...
		$selectedDb = $data_source['value']['database'];
		$selectedModule = $data_source['value']['module'];
		$auditedReport = $data_source['value']['audit'];
		...
		//***AlineaSol Premium***//
		//***********************//
		$extraParams = array('isSubQuery' => $subQueryMode, 'treeMode' => $treeMode, 'data_source' => $data_source);
		$externalApiTables = asol_ReportsUtils::managePremiumFeature("externalApisReports", "reportFunctions.php", "getExternalApiFields", $extraParams);
		$returnedHtml = ($externalApiTables !== false ? $externalApiTables : null);
		//***********************//
		//***AlineaSol Premium***//
		//***********************//
