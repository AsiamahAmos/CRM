...
class asol_CommonUtils {
	...
	static public $common_version = "2.10.0";
	...
	static public function isFormsViewsInstalled() {
		...
		if ( isset($_SESSION ['isFormsViewsInstalled']) && $_SESSION ['isFormsViewsInstalled']) {
	...
	static public function isReportsInstalled() {
		...
		if (isset($_SESSION['isReportsInstalled']) && $_SESSION['isReportsInstalled']) {
	 ...
	 * @return boolean
	 */
	static public function isCalendarEventsInstalled() {
		
		global $db;
		
		if ($_SESSION['isCalendarEventsInstalled']) {
			
			$isCalendarEventsInstalled = $_SESSION['isCalendarEventsInstalled'];
		} else {
			$calendarEventsQuery = $db->query ( "SELECT name FROM upgrade_history WHERE id_name='AlineaSolCalendarEvents' AND status='installed' AND enabled=1" );
			$isCalendarEventsInstalled = $_SESSION['isCalendarEventsInstalled'] = ($calendarEventsQuery->num_rows > 0);
		}
		
		return $isCalendarEventsInstalled;
	 ...
	 */
	public static function sortAssocArray(& $assocArray, $key, $isAssoc = true, $ascending = true, $isInteger = true) {
	...
	static public function getBlockDivs($loadingLabel = false, $savingLabel = false, $module = 'asol_Common') {
		...
		<div id="loadingBlockDiv" style="display: none">
			<i class="loadingGIF icn-loading" '.(isset($loadingLabel) ? '' : 'style="font-size:3em"').'></i>
		...
		<div id="savingBlockDiv" style="display: none">
			<i class="loadingGIF icn-loading" '.(isset($savingLabel) ? '' : 'style="font-size:3em"').'></i>
			'.(isset($savingLabel) ? '<span style="display:inline" id="savingText">&nbsp;'.translate($savingLabel, $module).'</span>' : '').'
		</div>';
	
	}
	
	static public function getLoadJqueryScript($loadBlock = false, $loadUI = false, $callback = null, $dynamicLoad = true) {
			
		if ($dynamicLoad) {
		
			$script = '<script type="text/javascript">if (typeof jQuery === "undefined") {
				$LAB.script("modules/asol_Common/include/client/libraries/asolJquery.js").wait('.($callback !== null ? $callback : '').');
			}';
			
			if ($loadBlock) {
				$script .= 'else if (typeof jQuery.blockUI === "undefined") {
			...
			$script = '<script type="text/javascript" src="modules/asol_Common/include/client/libraries/asolJquery.js"></script>';
			if ($callback !== null) {
				$script .= '<script type="text/javascript"> '.$callback.'(); </script>';
			}
	...
	static public function getRelationshipsInfoList($usedModule, $lhsModule, $rhsModule, $lhsTable, $rhsTable, $lhsKey, $rhsKey, $relationship_name = null) {
			...
			if (isset($beanList[$usedModule]) && $beanList[$usedModule]) {
				...
				foreach ($getLinkedFields as $relationship) {
					if ($mod->load_relationship($relationship['name'])) {
						...
						$relNameApply = (!isset($relationship_name) || $rel_object->def['name'] == $relationship_name);
						
						if (!isset($processedRels[$rel_object->def['name']])) {
	
							if ($relModulesApply && $relTablesApply && $relKeysApply && $relNameApply) {
	...
	static public function getModuleRelationShips($module, $field, $direction = null) {
		
		global $sugar_config;
		
		if (!isset($direction) || ($direction === 'left')) {
			$rsRightRel = asol_CommonUtils::getRelationshipsInfoList($module, null, $module, null, null, null, $field);
			if (empty($rsRightRel)) {
				$rsRightRel = asol_CommonUtils::getSelectionResults("SELECT DISTINCT relationship_name, lhs_module as main_module, lhs_module, lhs_table FROM relationships WHERE rhs_module LIKE '".$reportModule."' AND rhs_key LIKE '".$field."'");
			}
		}
		if (!isset($direction) || ($direction === 'right')) {
			$rsLeftRel = asol_CommonUtils::getRelationshipsInfoList($module, $module, null, null, null, $field, null);
			if (empty($rsLeftRel)) {
				$rsLeftRel = asol_CommonUtils::getSelectionResults("SELECT DISTINCT relationship_name, lhs_module as main_module, rhs_module, rhs_table FROM relationships WHERE lhs_module LIKE '".$reportModule."' AND lhs_key LIKE '".$field."'");
			}
		}
		
		foreach ($rsLeftRel as $leftRel) { //Avoid duplicated tables
	...
	static public function getHeaderNonGregorianCalendar() {
		...
		if( !isset($_REQUEST['asolLoadedNonGregorianCalendar']) ){
			...
			//JSs base
			//Ya tenemos JQuery?
			$header.= '<script type="text/javascript" src="modules/asol_Common/include/client/libraries/calendars/calendarJqBundle.js?version='.str_replace('.', '', self::$common_version).'"></script>';
	...
	public static function getAvailableReferences($module, $record) {
		...
		if ($module == 'asol_Reports') {
			...
			$reportFields = json_decode(rawurldecode($reportBean->report_fields), true);
			$reportFilters = json_decode(rawurldecode($reportBean->report_filters), true);
			$reportCharts = json_decode(rawurldecode($reportBean->report_charts_detail), true);
			
			foreach ($reportFields['tables'][0]['data'] as $currentField) {
				if (!empty($currentField['fieldReference'])) {
					$availableReferences['fields'][$currentField['fieldReference']] = $currentField['alias'];
				}
			}
			
			foreach ($reportFilters['data'] as $currentFilter) {
				if (!empty($currentFilter['filterReference'])) {
					$availableReferences['filters'][$currentFilter['filterReference']] = $currentFilter['alias'];
				}
			}
			
			foreach ($reportCharts['charts'] as $currentChart) {
				if (!empty($currentChart['reference'])) {
					$availableReferences['charts'][$currentChart['reference']] = $currentChart['alias'];
				}
			}
				...
				foreach ($currentPanel['rows'] as $currentRow) {
					foreach ($currentRow['fields'] as $currentField) {
						if ($currentField['editable'] && !empty($currentField['reference'])) {
							$availableReferences['fields'][$currentField['reference']] = $currentField['alias']['text'];
						}
					}
				}
			}
			...
			if ($domainsBean->asol_domain_enabled) {
				...
				if ($domainField !== null) {
					...
					if ($domainField != "") {
						...
						if (($_SESSION['asol_only_my_domain'] == 0) || ($passThrough)) {
							//***asol_child_domains***//
			...
			$returnedHtml .= '<div id="asolCommonAvailableDomainsSelection" style="display: none">
								...
								<table class="list view">
									<tbody>
										...
										<tr>
											<td><div id="commonAvailableDomainsDiv">'.asol_manageDomains::getChildDomainsSelectHtml($commonDomainId, 'icn-plus-circled', $switchDomainCallBack).'</div></td>
										</tr>
									</tbody>
								</table>
								<table class="list view">
	...
	public static function getElementByDomainSelectorHtml($usedDomains, $initDisabled, $containerClass, $changeDomainCallBack) {
		...
		if (self::isDomainsInstalled()) {
			...
			$domainViewPanelSelector .= '&nbsp;<i class="icn-globe asol_icon domains" title="'.translate('LBL_COMMON_MANAGE_DOMAIN_TITLE', 'asol_Common').'" class="asol_icon clickable domains" style="vertical-align: sub; display: '.($initDisabled ? 'none' : 'inline').';" onClick="openDomainManagementElementPanel(\''.$commonDomainId.'\', this, '.(isset($containerClass) ? "'".$containerClass."'" : 'null').');"></i>';
...
?>';
	...
	public static function getCurrentUserRolesNames() {
	
		global $current_user;
	
		if (isset($_SESSION['asolCommonUserRolesNames'])) {
			$currentUserRoles = $_SESSION['asolCommonUserRolesNames'];
		} else {
			$currentUserRoles = array();
			foreach (ACLRole::getUserRoles($current_user->id, false) as $role) {
				$currentUserRoles[] = $role->name;
			}
			$_SESSION['asolCommonUserRolesNames'] = $currentUserRoles;
		}
	...
	public static function getCurrentUserRolesIds() {
		...
		if(isset($_SESSION["authenticated_user_id"])){
			...
			if (isset($_SESSION['asolCommonUserRolesIds'])) {
				$currentUserRoles = $_SESSION['asolCommonUserRolesIds'];
			} else {
				foreach (ACLRole::getUserRoles($current_user->id, false) as $role) {
					$currentUserRoles[] = $role->id;
				}
				
				$_SESSION['asolCommonUserRolesIds'] = $currentUserRoles;
			}
			
		}else $_SESSION['asolCommonUserRolesIds'] =  null;
		
		return $currentUserRoles;
	}	
	
	public static function checkRolesInCurrentUser($roles) {
		
		$isInteger = (intval($roles[0]) !== 0);
		$userRoles = ($isInteger ? self::getCurrentUserRolesIds() : self::getCurrentUserRolesNames());
	...
	public static function getModuleMenus($conf, & $menu) {
		...
		$currentUserRoles = self::getCurrentUserRolesIds();
		...
		foreach ($conf["menus"] as $currentMenu) {
			...
			//***AlineaSol Premium***//
			//***********************//
			$extraParams = array('properties' => $currentMenu["properties"], 'elementDomain' => null);
	...
	public static function getModuleList(){
		...
		$modules  = array_keys(ACLAction::getUserActions($current_user->id));
		
		foreach ($modules as $key => $module){
			
			$modules[$module] = (isset($app_list_strings['moduleList'][$module])) ? $app_list_strings['moduleList'][$module] : $module;
			unset($modules[$key]);
			
		}
		uasort($modules, create_function('$a, $b', 'return strcmp($a, $b);'));
	...
	public static function executeModuleMenu($type, $id, $url, $title, $javascript) {
		...
		if (!empty($type)) {
			...
			if (($type == 'asol_Reports') && self::isReportsInstalled()) {
				...
				echo '<div class="alineasol_reports">'.asol_ControllerReportDetail::display($id, true, true).'</div>';
				...
				echo '<div class="alineasol_forms">'.asol_ControllerFormDetail::display($id, true, true).'</div>';
				...
				echo '<div class="alineasol_views">'.asol_ControllerViewDetail::display($id, true, true).'</div>';
	...
	static public function ajaxTracker($record, $name, $module, $action = 'DetailView', $force = false) {
	...
	static public function get_recently_viewed(){
		
		global $current_user, $db;
		
		$result = $db->query("SELECT  distinct item_summary, item_id, module_name, action FROM `tracker` WHERE id IN (SELECT MAX(id) FROM tracker GROUP BY item_id) and user_id = ".$current_user->id." ORDER BY date_modified DESC LIMIT 10");
		...
		if ($result->num_rows > 0) {
			...
			while ($row = $db->fetchByAssoc($result) ) {
				...
				$row['image'] = SugarThemeRegistry::current()->getImage($row['module_name'],'border="0" align="absmiddle"',null,null,'.gif',$row['item_summary']);
	...
	public static function hasPreviewExecution($modulePrefix) {
		...
		if (($previewExecutionScope === 0) || (($previewExecutionScope === 1) && (!$current_user->is_admin))) {
			...
			$userRoles = self::getCurrentUserRolesNames();
	...
	public static function dateToPersian($date, $hasTime, $dateFormat) {
	...
	public static function currency_format($number) {
		...
		if (isset($sugar_config['asolCommonCurrencyUsage'])) {
			
			if (self::isDomainsInstalled()) {
				$currentDomainID = $current_user->asol_default_domain;
				if (isset($sugar_config['asolCommonCurrencyUsage']['domains'][$currentDomainID])) {
					$iso4217 = $sugar_config['asolCommonCurrencyUsage']['domains'][$currentDomainID];
				} else {
					$iso4217 = $sugar_config['asolCommonCurrencyUsage']['instance'];
				}
			} else {
				$iso4217 = $sugar_config['asolCommonCurrencyUsage']['instance'];
			}
			
			$currencySymbol = $sugar_config['asolCommonCurrencyConfig'][$iso4217]['symbol'];
			
		} else {
		
			if ($current_user->getPreference('currency')) {
				$currentCurrency = new Currency();
				$currentCurrency->retrieve($current_user->getPreference('currency'));
				$currencySymbol = $currentCurrency->symbol;
				$iso4217 = $currentCurrency->iso4217;
			} else {
				$currencySymbol = $sugar_config['default_currency_symbol'];
				$iso4217 = $sugar_config['default_currency_iso4217'];
			}
		
		}
		
		if (isset($sugar_config['asolCommonCurrencyConfig'][$iso4217])) {
			$currencySymbol = $sugar_config['asolCommonCurrencyConfig'][$iso4217]['symbol'];
			$positionCurrencySymbol = $sugar_config['asolCommonCurrencyConfig'][$iso4217]['position'];
		}
		
		$currencySymbol = $locale->translateCharset($currencySymbol, 'UTF-8', $locale->getExportCharset());
		...
		if (isset($sugar_config['asolCommonCurrencyConfig'][$iso4217]['hasCents']) && !$sugar_config['asolCommonCurrencyConfig'][$iso4217]['hasCents']) {
			$params['decimals'] = 0;
		}
	...
	function getJsLanguages($module = null) {
		
		global $sugar_config, $current_user, $app_list_strings;
		
		$asolCacheDir = '';
		
		if (isset($sugar_config['asolInstallationName'])) {
			$instanceName = $sugar_config['asolInstallationName'];
		...
		foreach ($modules as $key => $module) if (!isset($app_list_strings['moduleList'][$module])) unset($modules[$key]);
		...
		foreach ($modules as $module) {
			...
			if (!is_file(sugar_cached('jsLanguage/').$asolCacheDir.$module."/".$GLOBALS['current_language'].".js")) {
				...
				$cacheDir = create_cache_directory('jsLanguage/'.$asolCacheDir.$module.'/');
				$jsLang = fopen($cacheDir.$GLOBALS['current_language'].'.js', 'a+');
				
				fwrite($jsLang, file_get_contents("cache/jsLanguage/".$asolCacheDir.$module."/".$GLOBALS['current_language'].".js"));
			}
			
			$returnedScripts .= '<script src="cache/jsLanguage/'.$asolCacheDir.$module.'/'.$GLOBALS['current_language'].'.js?v='.md5_file('cache/jsLanguage/'.$asolCacheDir.$module.'/'.$GLOBALS['current_language'].'.js').'"></script>';
	...
	public static function getRtlLanguages() {
		...
		$defaultRtlLangs = array(
				'ar_SA',
				'fa_IR',
				'he_IL',
		);
					
		$rtlLangs = isset($sugar_config['asolThemeRtlLanguages']) ? array_merge($defaultRtlLangs, $sugar_config['asolThemeRtlLanguages']) : $defaultRtlLangs;
