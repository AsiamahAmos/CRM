...
class ReportChartDashlet extends DashletGenericChart {
    ...
    public function displayOptions() {
		...
		//***AlineaSol Premium***//
		//***********************//
		$availableDatabases = asol_ReportsUtils::managePremiumFeature("externalDatabasesReports", "reportFunctions.php", "getExternalDatabasesInfo", null);
		//$availableApis = asol_ReportsUtils::managePremiumFeature("externalApisReports", "reportFunctions.php", "getExternalApisInfo", null);
		//***********************//
		//***AlineaSol Premium***//
		//***********************//
	    ...
	    $modulesSelect .= 	"<option value=''></option>";
	    $modulesSelect .= 	"<option value='-1' ".($sDatabase == '-1' ? 'selected' : '').">CRM</option>";
		foreach ($availableDatabases as $db_index => $currentDatabase) {
			$modulesSelect .= "<option value='".$db_index."' ".($sDatabase == $db_index ? 'selected' : '').">".$currentDatabase."</option>";
		}
		...
		$scopesSelect .= 	"<option value='' ".(empty($sScope) ? 'selected' : '').">".$dashletStrings['ReportChartDashlet']['LBL_REPORT_SCOPE_ALL']."</option>";
		$scopesSelect .= 	"<option value='public' ".($sScope == "public" ? 'selected' : '').">".$dashletStrings['ReportChartDashlet']['LBL_REPORT_SCOPE_PUBLIC']."</option>";
		$scopesSelect .= 	"<option value='private' ".($sScope == "private" ? 'selected' : '').">".$dashletStrings['ReportChartDashlet']['LBL_REPORT_SCOPE_PRIVATE']."</option>";
		$scopesSelect .= 	"<option value='role' ".($sScope == "role" ? 'selected' : '').">".$dashletStrings['ReportChartDashlet']['LBL_REPORT_SCOPE_ROLE']."</option>";
		$scopesSelect .= "</select>";
