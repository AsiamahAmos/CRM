...
class asol_ReportsManagementFunctions {
	...
	static public function getFilteredReportList($sDatabase = '-1', $sModule = null, $sScope = null, $sName = null, $sUser = null, $sMeta = null) {
		...
		if (isset($sDatabase) && $sDatabase !== "") {
			$sqlReport .= " AND (asol_reports.data_source LIKE '%".rawurlencode('"database":"'.$sDatabase.'"')."%')";
		}
		if (!isset($sModule) || $sModule === "") {
			$sqlReport .= " AND (asol_reports.data_source IS NULL OR (asol_reports.data_source LIKE '%%'".($sDatabase == '-1' ? " AND (asol_reports.data_source REGEXP ('".$sqlModules."'))))" : "))");
		} else {
			$sqlReport .= " AND (asol_reports.data_source LIKE '%".rawurlencode('"module":"'.$sModule.'"')."%')";
		}
		...
		$sqlReport .= " AND asol_reports.report_type NOT LIKE '".rawurlencode('{"draft":"1"}')."' ";
	...
	static public function prepareReportFields($data_source, $fieldsJson, $translateFieldLabels, $fieldsToBeRemoved) {
		
		$fieldsArray = json_decode(rawurldecode($fieldsJson), true);
		
		//***********************//
		//***AlineaSol Premium***//
		//***********************//
		$subQueryFieldsModifiedEnums = asol_ReportsUtils::managePremiumFeature("subQuerySQLEditor", "reportFunctions.php", "getSubQueryEnumFilters", array('data_source' => $data_source, 'fields' => $fieldsArray, 'translateFieldLabels' => $translateFieldLabels, 'fieldsToBeRemoved' => $fieldsToBeRemoved));
	...
	static public function prepareReportFilters($data_source, $filtersJson, $translateFieldLabels, $fieldsToBeRemoved) {
		...
		$filtersArray = json_decode(rawurldecode($filtersJson), true);
		...
		foreach ($filtersArray['data'] as & $currentFilter) {
			...
			if (in_array($currentFilter['enumOperator'], array('options', 'function'))) {
				...
				$currentFilter['enumValues'] = asol_Reports::getEnumValues($data_source, $currentFilter['field'], $currentFilter['enumOperator'], $currentFilter['enumReference']);
				$currentFilter['enumLabels'] = asol_Reports::getEnumLabels($data_source, $currentFilter['field'], $currentFilter['enumOperator'], $currentFilter['enumReference']);
				
			} else if ($currentFilter['enumOperator'] == 'asolFunction') {
				
				$currentFilter['enumLabels'] = ($translateFieldLabels) ? asol_Reports::getAuditedLabels($data_source['value']['module'], $fieldsToBeRemoved) : asol_Reports::getAuditedFields($data_source['value']['module'], $fieldsToBeRemoved);
				$currentFilter['enumValues'] = asol_Reports::getAuditedFields($data_source['value']['module'], $fieldsToBeRemoved);
	...
	static public function prepareReportTasks($reportTasks, $dateModified, $return_action) {
		...
		if ((!isset($_REQUEST['report_tasks'])) || ($return_action == 'duplicate')) {
	...
	static public function prepareReportCharts($report_charts) {
		...
		$selected_charts = json_decode(rawurldecode($report_charts), true);
	...
	static public function getAuditTableFields($bean, $fieldsToBeRemoved, & $relateField, $mainRelatedKey) {
		...
		if (empty($relateField)) {
			...
			$availableFields[] = array('field' => 'id', 'label' => ($translateFieldLabels ? $app_strings['LBL_ID'] : 'id'), 'type' => 'char(36)', 'enum' => null, 'index' => false, 'link' => false);
			$availableFields[] = array('field' => 'parent_id', 'label' => ($translateFieldLabels ? translate("LBL_AUDIT_REPORT_PARENT_ID", "asol_Reports") : 'parent_id'), 'type' => 'relate', 'enum' => null, 'index' => false, 'link' => true);
			$availableFields[] = array('field' => 'date_created', 'label' => ($translateFieldLabels ?translate("LBL_DATE_ENTERED", "Audit") : 'date_created'), 'type' => 'datetime', 'enum' => null, 'index' => false, 'link' => false);
			$availableFields[] = array('field' => 'created_by', 'label' => ($translateFieldLabels ? translate("LBL_CREATED_BY", "Audit") : 'created_by'), 'type' => 'relate', 'enum' => null, 'index' => false, 'link' => ($allowedModules['Users'] || $current_user->is_admin));
			$availableFields[] = array('field' => 'field_name', 'label' => ($translateFieldLabels ? translate("LBL_FIELD_NAME", "Audit") : 'field_name'), 'type' => 'varchar(100)', 'enum' => null, 'index' => false, 'link' => false);
			$availableFields[] = array('field' => 'data_type', 'label' => ($translateFieldLabels ? translate("LBL_AUDIT_REPORT_DATA_TYPE", "asol_Reports") : 'data_type'), 'type' => 'varchar(100)', 'enum' => null, 'index' => false, 'link' => false);
			$availableFields[] = array('field' => 'before_value_string', 'label' => ($translateFieldLabels ? translate("LBL_OLD_NAME", "Audit") : 'before_value_string'), 'type' => 'varchar(255)', 'enum' => null, 'index' => false, 'link' => false);
			$availableFields[] = array('field' => 'after_value_string', 'label' => ($translateFieldLabels ? translate("LBL_NEW_VALUE", "Audit") : 'after_value_string'), 'type' => 'varchar(255)', 'enum' => null, 'index' => false, 'link' => false);
			$availableFields[] = array('field' => 'before_value_text', 'label' => ($translateFieldLabels ? translate("LBL_OLD_NAME", "Audit") : 'before_value_text'), 'type' => 'text', 'enum' => null, 'index' => false, 'link' => false);
			$availableFields[] = array('field' => 'after_value_text', 'label' => ($translateFieldLabels ? translate("LBL_NEW_VALUE", "Audit") : 'after_value_text'), 'type' => 'text', 'enum' => null, 'index' => false, 'link' => false);
			...
			foreach($rsrf as $val) {
				...
				$field = (count(explode(".", $val['Field'])) == 1) ? $relatedTable.".".$val['Field'] : $val['Field'];
				
				$label = (empty($fieldInfo['label'])) ? $currentRelatedField : $fieldInfo['label'];
				$tableVname = (!empty($app_list_strings['moduleList'][$relatedModule]) ? $app_list_strings['moduleList'][$relatedModule] : $relatedTable);
				$label = (count(explode(".", $val['Field'])) >= 1 ? $tableVname.".".$label : $label);
				$label = trim($label);
				$label = (substr($label, -1) == ':') ? substr($label, 0, -1) : $label;
				...
				if ($currentRelatedField == $primaryKey) {
					...
					$link = ((count($rsrhr) > 0) || ($val['Type'] == "relate") ? true : false);
				...
				$availableFields[] = array(
					'field' => $field, 
					'label' => ($translateFieldLabels ? $label : $field), 
					'type' => $type, 
					'enum' => array(
						'operator' => $fieldInfo['enum']['operator'],
						'reference' => $fieldInfo['enum']['reference']
					), 
					'index' => false, 
					'link' => $link
				);
	...
	static private function getModuleRelationShips($module, $field) {
		...
		return asol_CommonUtils::getModuleRelationShips($module, $field);
		...
		foreach ($rsFields as $value) {
			...
			if ($value['Type'] != "non-db") {
				...
				if ($value['Field'] == $primaryKey) { // CRM RelationShips
					...
					$label = ($translateFieldLabels) ? $app_strings["LBL_ID"] : $primaryKey;
					$link = ((count($rshr) > 0) || ($value['Type'] == "relate") ? true : false);
					...
					if (!empty($relatedInfo)) { // CRM Relate Fields
						...
						foreach ($relatedInfo as $info) {
							...
							if ($infoIdName == $currentField) {
								...
								$label = ($translateFieldLabels) ? self::getTranslatedField($reportModule, $value['Field'], $info['vname']) : $value['Field'];
								$type = "relate";
								$link = ((isset($allowedModules[$info['module']]) && $allowedModules[$info['module']]) || $current_user->is_admin ? true : false);
								...
								$label = ($translateFieldLabels) ? self::getTranslatedField($reportModule, $value['Field'], $normalInfo['vname']) : $value['Field'];
								$link = ($isVirtualLink ? true : false);
		...
		if ($currentField == $primaryKey) { // CRM RelationShips
			...
			if (count($rshr) > 0) {
				...
				while ($j < count($rshr)) {
					...
					if ((isset($allowedModules[$relatedModule]) && $allowedModules[$relatedModule]) || $current_user->is_admin) {
						...
						foreach($rsrf as $val) {
							...
							$field = (count(explode(".", $val['Field'])) == 1) ? $relatedTable.".".$val['Field'] : $val['Field'];
							$label = ($translateFieldLabels) ? self::getTranslatedRelatedField($relatedModule, $relatedTable, $currentRelatedField, $fieldInfo['label']) : $field;
							$type = $fieldInfo['type'];
							...
							$keys[$k] = $primaryKey." ".$rshr[$j]['relationship_name'];
							...
							$availableFields[] = array(
								...
								'link' => $link,
								'relation' => array(
									'value' => $rshr[$j]['relationship_name'],
									'label' => ($translateFieldLabels ? asol_Reports::getRelationShipLabelFromVardefs($rshr[$j]['main_module'], $rshr[$j]['relationship_name']) : $rshr[$j]['relationship_name'])
								)
							);
				...
				foreach($rsrf as $val) {
					...
					$field = $relatedTable.".".$val['Field'];
					$label = $relatedTable.".".$val['Field'];
					$type = $fieldInfo['type'];
			...
			if ((isset($allowedModules[$relatedModule]) && $allowedModules[$relatedModule]) || $current_user->is_admin) {
				...
				foreach($rsrf as $val) {
					...
					$availableFields[] = array(
						...
						'relation' => array(
							'value' => $relatedModule,
							'label' => (!empty($app_list_strings['moduleList'][$relatedModule]) ? $app_list_strings['moduleList'][$relatedModule] : $relatedTable)
						)
					);
	...
	static public function getHeaderLinksHtml() {
		...
		return '
		<link rel="stylesheet" type="text/css" href="modules/asol_Reports/include/css/style.css?version='.$reportsVersion.'">
		
		<script type="text/javascript" src="modules/asol_Reports/include/client/helpers/reportsRouter.js?version='.$reportsVersion.'"></script>
		<script type="text/javascript" src="modules/asol_Reports/include/client/helpers/reportsApi.js?version='.$reportsVersion.'"></script>
		<script type="text/javascript">
		...
		<script type="text/javascript" src="modules/asol_Reports/include/client/controllers/controllerDetail.js?version='.$reportsVersion.'"></script>
		<script type="text/javascript">	
		...
		<script type="text/javascript" src="modules/asol_Common/include/client/libraries/asolFormatEditor.js?version='.$commonVersion.'"></script>
		<script type="text/javascript">
			...
			asolFormatEditor.setLanguage({
				"type" : {
					...
					"password" : "'.translate("LBL_FORMAT_PASSWORD", "asol_Common").'",
					"multimedia" : "'.translate("LBL_FORMAT_MULTIMEDIA", "asol_Common").'",
					"color" : "'.translate("LBL_FORMAT_COLOR", "asol_Common").'",
					"text" : "'.translate("LBL_FORMAT_TEXT", "asol_Common").'",
					"file" : "'.translate("LBL_FORMAT_FILE", "asol_Common").'"
				},
				"icon": {
					"set" : "'.translate("LBL_COMMON_SETICON", "asol_Common").'",
					"search" : "'.translate("LBL_COMMON_SEARCH", "asol_Common").'",
					"rotate" : "'.translate("LBL_COMMON_ROTATE", "asol_Common").'",
					"other" : "'.translate("LBL_COMMON_OTHER_IMG", "asol_Common").'",
					"save" : "'.translate("LBL_COMMON_SAVE", "asol_Common").'",
					"clear" : "'.translate("LBL_COMMON_CLEAR", "asol_Common").'",
					"cancel" : "'.translate("LBL_COMMON_CANCEL", "asol_Common").'",
				}
			});
		</script>
		...
		<script type="text/javascript" src="modules/asol_Common/include/client/libraries/LAB.min.js?version='.$commonVersion.'"></script>
		<script type="text/javascript" src="modules/asol_Common/include/client/libraries/jquery.zclip.min.js?version='.$commonVersion.'"></script>
		<script type="text/javascript" src="modules/asol_Common/include/client/libraries/chromajs/chroma.js?version='.$commonVersion.'"></script>
		...
		function initJqueryScripts(loadPlugins, callback) {
			...
			if (loadPlugins) {
				...
				if (typeof jQuery === "undefined") {
					...
					$LAB.script("modules/asol_Common/include/client/libraries/asolJquery.js");
				...
				if (typeof jQuery === "undefined") {
					...
					$LAB.script("modules/asol_Common/include/client/libraries/asolJquery.js").wait(callback);
	...
	static public function getInitDragDropElementsHtml() {
		...
		function initDragDropElements() {
			...
			<!-- Fields -->
			asolJquery("#fields_Table").sortable({ items: ".asolReportsFieldRow", axis: "y" });
			
			<!-- Filters -->
			asolJquery("#filters_Table .asolReportsFilterWhere").sortable({ items: ".asolReportsFilterRow", axis: "y" });
			asolJquery("#filters_Table .asolReportsFilterHaving").sortable({ items: ".asolReportsFilterRow", axis: "y" });
			
			<!-- Charts -->
			asolJquery("#charts_Table").sortable({ items: ".asolReportsChartsGroup", axis: "y" });
		
			<!-- Tasks -->
			asolJquery("#tasks_Table").sortable({ items: ".asolReportsTaskRow", axis: "y" });
	...
	static public function getOnloadJavaScript() {
		...
		function loadReportsManagementJavaScript() {
			initJqueryScripts(true, function() {	
				$.blockUI({ message: reportsApi.getBlockingMessage("load") });
	...
	static public function getReportsJQueryScript() {
		...
		return !asol_CommonUtils::isDomainsInstalled() ? '<script type="text/javascript" src="modules/asol_Common/include/client/libraries/jstree/jquery.jstree.js"></script>' : '';
	...
	static public function getFieldsSelectHtml($database, $selectedModule, $hasDeleted, $selectedField, $auditedReport = 0, $isEditViewMode = true, $isDisabled = false, $treeMode = false, $subQueryMode = false) {
		...
		if (($database >= 0) && ($selectedModule != '')) {
			...
			if ($currentTableFields !== false) {
				$rhs_key = $currentTableFields['key'];
				$currentTableFields = $currentTableFields['fields'];
			}
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
		...
		asol_CommonUtils::sortAssocArray($currentTableFields, 'label', false, true, false);
		...
		if ($treeMode) {
			...
			foreach ($currentTableFields as $index => $field) {
				$returnedHtml .= '<option '.($field['link'] ? 'style="color:blue;"' : '').' '.($field['field'] == $selectedField ? 'selected' : '').' onContextMenu="getReportVariableHover(\''.$field['field'].'\', \'\', \'\', this); return false;" title="'.$field['field'].'" value="'.$field['field'].'" fieldValue="'.$field['field'].'" fieldIndex="'.$index.'" fieldType="'.$field['type'].'" fieldEnumOp="'.$field['enum']['operator'].'" fieldEnumRef="'.$field['enum']['reference'].'" fieldLabel="'.$field['label'].'" fieldModule="" fieldKey="">'.$field['label'].($field['link'] ? ' +' : '').'</option>';
			}
			$returnedHtml .= '</select>';
			...
			foreach ($currentTableFields as $index => $field) {
				...
				if ($currentModule != $previousModule) {
					...
					if ($aux_counter + 1 != count($fields)) {
						...
						$fields_label_array = explode(".", $field['label']);
						$aux_current_module_label = $fields_label_array[0];
						
						if (($aux_current_module_label == $field['relation']['label']) || ($field['relation']['label'] == ""))
							$returnedHtml .= '<optgroup title="'.$aux_current_module_label.'" label="'.$aux_current_module_label.'">';
						else
							$returnedHtml .= '<optgroup title="'.$aux_current_module_label.' ('.$field['relation']['label'].')" label="'.$aux_current_module_label.' ('.$field['relation']['label'].')">';
						
					}
					
				}
	
				$fieldLabelExploded = explode(".", $field['label']);
				$field['field'] = (!empty($mainRelatedTable) ? $mainRelatedTable.'::'.$field['field'] : $field['field']);
				$fieldExploded = explode(".", $field['field']);
				...
				$isRelated = ($field['link'] && $getRelateLinks);
	
				$returnedHtml .= '<option '.($isRelated ? 'style="color: '.$linkColor.';"' : '').' onContextMenu="getReportVariableHover(\''.$field['field'].'\', \''.$fieldModule.'\', \''.$fieldKey.'\', this); return false;" title="'.$field['field'].'" value="'.$field['field'].'" fieldValue="'.$field['field'].'" fieldIndex="'.$index.'" fieldType="'.$field['type'].'" fieldEnumOp="'.$field['enum']['operator'].'" fieldEnumRef="'.$field['enum']['reference'].'" fieldLabel="'.$field['label'].'" fieldModule="'.$fieldModule.'" fieldKey="'.$fieldKey.'">'.$fieldLabelExploded[1].($isRelated ? ' +' : '').'</option>';
	...
	static public function getFieldsHeadersHtml($selectedDb, $rowIndexDisplay) {
			...
			<thead>
					...
					<td colspan="10">
						<h4 style="border-radius:0">
							<div>
								...
								<i title="'.$mod_strings['LBL_REPORT_TABLE_CONFIGURATION'].'" class="icn-sliders configure_table_btn" onclick="showTableConfig(\'tableConfiguration\');"></i>
								<i title="'.$mod_strings['LBL_REPORT_EXTERNAL_FILTER_REFERENCE'].'"	class="icn-attach right references_btn" onclick="showReferencesFields();"></i>';		
								//***********************//
								//***AlineaSol Premium***//
								//***********************//
				...
				<tr>
					...
					<th nowrap="nowrap" scope="col" class="aliasHeader">
						...
						<i title="'.$mod_strings['LBL_REPORT_FIELD_ORDERING'].'" class="icn-sort-number reorder_fields_btn" onclick="showFieldOrdering();"></i>';
		...
		$returnedPremiumTargetButton= asol_ReportsUtils::managePremiumFeature("getTargetButton", "reportFunctions.php", "getTargetButton", null);
		$returnedHtml .= ($returnedPremiumTargetButton !== false) ? $returnedPremiumTargetButton : '';	
		//***********************//
		//***AlineaSol Premium***//
		//***********************//
		...
		$returnedHtml .= '
					...
					<th nowrap="nowrap" scope="col" class="groupByHeader">
						...
						<i title="'.$mod_strings['LBL_REPORT_GROUP_ORDERING'].'" class="icn-sort-number reorder_grouping_btn" onclick="showGroupOrdering();"></i>
						</div>
					</th>
			...
			<tbody>
				<tr class="asolReportsFieldsIndexRow'.($rowIndexDisplay == '1' ? '' : ' hiddenRow').'">
					...
					<td align="right">
						...
						<i title="'.$mod_strings['LBL_REPORT_VISIBLE'].'" class="index_visible icn-eye clickable" style="display: '.($rowIndexDisplay == '1' ? 'inline' : 'none').';"></i>
						<i title="'.$mod_strings['LBL_REPORT_HIDDEN'].'" class="index_hidden icn-eye-off clickable" style="display: '.($rowIndexDisplay == '1' ? 'none' : 'inline').';"></i>	
					</td>
				</tr>
			</tbody>
		</table>
		...
		//***AlineaSol Premium***//
		//***********************//
		$returnedCalculationModeHtml = asol_ReportsUtils::managePremiumFeature("calculationMode", "reportFunctions.php", "getReportCalculationModeHtml", null);
			...
			<thead>
				<tr>
					<td colspan="8">
							...
							<i title="'.$mod_strings['LBL_REPORT_FILTERS_CONFIGURATION'].'" class="icn-sliders configure_filters_btn" onclick="showFiltersConfig();"></i>
							<i title="'.$mod_strings['LBL_REPORT_EXTERNAL_FILTER_REFERENCE'].'" class="icn-attach right references_btn" onclick="showReferencesFilters();"></i>';
							//***********************//
							//***AlineaSol Premium***//
							//***********************//
			...
			<table class="edit view edit508">
				<tbody>
					...
					<tr>
						...
						//***AlineaSol Premium***//
						//***********************//
						$returnedSaveSearchHtml= asol_ReportsUtils::managePremiumFeature("getReportSaveSearchHtml", "reportFunctions.php", "getReportSaveSearchHtml", null);
						$returnedHtml .= ($returnedSaveSearchHtml !== false) ? $returnedSaveSearchHtml : '';
						//***********************//
						//***AlineaSol Premium***//
						//***********************//
		$returnedHtml .=
			...
			<table class="edit view edit508">
				<tbody>
					...
					<tr>
						<td><input type="checkbox" id="disallow_trim"> '.$mod_strings['LBL_REPORT_AVOID_TRIM_SEARCH'].'</td>
					</tr>
					<tr>
						<td><input type="checkbox" id="avoid_empty_filters"> '.$mod_strings['LBL_REPORT_AVOID_EMPTY_FILTERS'].'</td>
					</tr>
					<tr>
						<td><input type="checkbox" id="avoid_autocomplete"> '.$mod_strings['LBL_REPORT_AVOID_AUTOCOMPLETE'].'</td>
					</tr>
				</tbody>
			</table>';
		...
		$returnedHtml .= '
			<table class="edit view edit508">
				<tbody>
					...
					<tr>
						<td><input type="checkbox" id="allow_bookmarks"> '.$mod_strings['LBL_REPORT_ALLOW_BOOKMARKS'].'</td>
					</tr>
				</tbody>
			</table>
				...
				<tr>
					<td colspan="10">
						<h4>
						    ...
						    <i class="icn-sliders chartLayout" title="'.$mod_strings['LBL_REPORT_CHART_LAYOUT_CONFIG'].'" onclick="layoutConfig(this);"></i>
							<i title="'.$mod_strings['LBL_REPORT_EXTERNAL_FILTER_REFERENCE'].'"	class="icn-attach right references_btn" onclick="showReferencesCharts();"></i>
							<i class="icn-plus-circled" title="'.$mod_strings['LBL_REPORT_ADD_CHART'].'" onClick="InsertChart(\'charts_Table\'); if (typeof window.hasPremiumReportsJsFeatures == \'function\') { applyChartsRestrictions(); };" style="float:right"></i>		
						</h4>
					</td>
				</tr>	
				<tr>
					...
					<th nowrap="nowrap" scope="col">
						<div align="left" width="100%" style="white-space: nowrap;">
						'.$mod_strings['LBL_REPORT_CHARTS_NAME'].' '.$multiLanguageChartsButton.'
						</div>
					</th>
					<th nowrap="nowrap" scope="col">
						<div align="left" width="100%" style="white-space: nowrap;">
		...
		<table class="list view" id="tasks_Table">
			<thead>
				<tr>
					...
					//***AlineaSol Premium***//
					//***********************//
					$returnedCronMode = asol_ReportsUtils::managePremiumFeature("cronMode", "reportFunctions.php", "getCronModeTableHead", $extraParams);
					$returnedHtml .= $returnedCronMode? $returnedCronMode: '';
					//***********************//
					//***AlineaSol Premium***//
					//***********************//
					$returnedHtml .= '<th nowrap="nowrap" scope="col" width="35%">
						<div align="center" width="100%" style="white-space: nowrap;">
							'.$mod_strings['LBL_REPORT_EXECUTION_NEXT_EXECUTION'].'
						</div>
					</th>
					<th nowrap="nowrap" scope="col">
						<div align="left" width="100%" style="white-space: nowrap;">
	...
	public static function getHiddenInputs($report_id, $rhs_key, $scheduledTypeInfo, $mySQLcheckInsecurity, $PHPcheckInsecurity, $availablePhpFunctionsJson, $predefinedTemplates, $dynamicTables, $dynamicSql, $isMeta, $metaHtml) {
		...
		<input type="hidden" value="" name="report_fields">
		<input type="hidden" value="" name="report_filters">
		<input type="hidden" value="" name="report_tasks">
		...
		<input type="hidden" value="'.rawurlencode(asol_ControllerReports::getEditReport($report_id)).'" id="asolReportsStoredContent">
		';
		...
		$availableMaps = asol_ReportsUtils::managePremiumFeature("manageMapCharts", "reportFunctions.php", "getAvailableMaps", null);
		$returnedHtml .= ($availableMaps!== false ? $availableMaps: '');
		//***********************//
		//***AlineaSol Premium***//
		//***********************//
	...
	public static function getReportDisplayOptionsHtml($report_charts, $isDisabled) {
		...
		<td nowrap="nowrap" width="35%">
			...
			<select '.($isDisabled ? 'disabled' : '').' name="report_charts" id="report_charts" onChange="manageReportChartsSelect(this.selectedIndex)">';
	...
	public static function getReportModuleTablesHtml($database, $subQuery = false, $selectedModule = null, $isDynamicTable = false, $auditedReport = 0, $selAutoRefresh = 0, $isEditViewMode = true, $isDisabled = false, $noRestrictions = false, $onChangeFunction = null) {
		...
		if ($database >= 0) {
			...
			$returnedHtml = '
			<select '.(($isDisabled || $isDynamicTable) ? 'disabled' : '').' id="'.($subQuery ? 'sub_report_module' : 'report_module').'" '.(!empty($onChangeFunction) ? 'onChange="'.$onChangeFunction.'"' : '').' '.($isEditViewMode ? 'onChange="$(\'#create_form\').find(\'[name=dynamic_tables]\').val(\'0\'); controllerReportEdit.manageDataSourceModule(\''.$treeMode.'\', \''.$fieldSelectorDivId.'\', true, false, true);"' : '').'>';
			...
			$returnedHtml = '<select '.($isDisabled ? 'disabled' : '').' id="'.($subQuery ? 'sub_report_module' : 'report_module').'" name="'.($subQuery ? 'sub_report_module' : 'report_module').'" '.(!empty($onChangeFunction) ? 'onChange="'.$onChangeFunction.'"' : '').' '.($isEditViewMode ? 'onChange="controllerReportEdit.manageDataSourceModule(\''.$treeMode.'\', \''.$fieldSelectorDivId.'\', true, false, true);"': '').'>';
	...
	static public function getReportAttachmentFormatHtml($generatedFileFormat, $isMetaReport, $isDisabled) {
		...
		$returnedHtml .= '<select '.($isDisabled ? 'disabled' : '').' id="report_attachment_format" name="report_attachment_format">';
	...
	static public function getReportTypeHtml($reportType, $reportTypeUri, $reportScheduledType, $isMetaReport, $isDisabled) {
		...
		$basicValues = array('manual', 'internal', 'external');
		$basicLabels = array('LBL_REPORT_MANUAL', 'LBL_REPORT_INTERNAL', 'LBL_REPORT_EXTERNAL');
		
		$extendedValues = array('scheduled', 'stored');
		$extendedLabels = array('LBL_REPORT_SCHEDULED', 'LBL_REPORT_STORED');
		
		$values = array_merge($basicValues, $extendedValues);
		$labels = array_merge($basicLabels, $extendedLabels);
		
		//***********************//
		//***AlineaSol Premium***//
		//***********************//
		$webServiceReportType = asol_ReportsUtils::managePremiumFeature("webServiceReports", "reportFunctions.php", "getWebServiceReportType", array('reportType' => $reportType['type'], 'isMetaReport' => 0));
		...
		<td nowrap="nowrap" width="35%">
			<select '.($isDisabled ? 'disabled' : '').' id="report_type" draft="'.(isset($reportType['draft']) ? $reportType['draft'] : '0').'" name="report_type" data="'.$reportTypeUri.'" onChange="if (typeof window.hasPremiumReportsJsFeatures == \'function\') { manageReportTypePremiumSelect(this.value, '.($isMetaReport == '1' ? 'true' : 'false').'); } else { manageReportTypeSelect(this.value); }">';
				foreach ($values as $key=>$value) {
					$returnedHtml .= '<option value="'.$value.'" '.($reportType['type'] == $value ? 'selected' : '').($isMetaReport == '1' && ( in_array($value, $extendedValues) || ( $webServiceReportType !== false && $value == $webServiceReportType['value'] ) ) ? ' style="display:none"': '').'>'.$mod_strings[$labels[$key]].'</option>';
				}
			$returnedHtml .= '
			</select>
			<select id="report_scheduled_type" name="report_scheduled_type" style="display: none;">';
	...
	static public function cleanUpStoredReportFiles($storedReportInfo, $domainIdsToDelete = null) {
		...
		return (!empty($storedInfo) ? base64_encode(serialize($storedInfo)) : null);
	...
	static public function manageImportReportValues(& $focus, $reportRow) {
		...
		$reportDatabase = $reportRow['alternative_database'];
		$reportModule = $reportRow['report_module'];
		$auditedReport = $reportRow['audited_report'];
			...
			switch ($fieldKey) {
				...
				case "report_fields":
					if ($focus->is_meta != 1) {
						...
						$focus->$fieldKey = asol_ReportsMigrationFunctions::migrateReportsBase64Columns($focus->$fieldKey);
					}
					break;
				case "report_filters":
					if ($focus->is_meta != 1) {
				...
				case "report_module":
					if ($focus->is_meta != 1) {
						$fieldValue = asol_ReportsMigrationFunctions::migrateReportExternalTable($fieldValue, $reportDatabase);
						$focus->data_source = asol_ReportsMigrationFunctions::migrateReportDataSource($fieldValue, $reportDatabase, $auditedReport);
					}
					break;
				case "report_type":
					$focus->$fieldKey = asol_ReportsMigrationFunctions::migrateReportTypeToSerialized($fieldValue);
					$focus->$fieldKey = asol_ReportsMigrationFunctions::migrateReportTypeToJson($focus->$fieldKey);
					break;
				case "internal_id":
					$focus->parent_id = $fieldValue;
				default:
	...
	static public function getDistributionListPanel($emailList) {
		...
		<ul class='yui-nav' id='emailTabs'>
			<li class='selected'><a href='javascript: void(0);' id='all_Tab' onClick='refreshEmailListReadonly(); controllerReportEdit.manageEditionTabs(\"emailTabs\", \"all\", \"emailPanel\");'><em>".$mod_strings['LBL_REPORT_SEND_EMAIL_ALL']."</em></a></li>
			<li><a href='javascript: void(0);' id='to_Tab' onClick='controllerReportEdit.manageEditionTabs(\"emailTabs\", \"to\", \"emailPanel\");'><em>".$mod_strings['LBL_REPORT_SEND_EMAIL_TO']."</em></a></li>
			<li><a href='javascript: void(0);' id='cc_Tab' onClick='controllerReportEdit.manageEditionTabs(\"emailTabs\", \"cc\", \"emailPanel\");'><em>".$mod_strings['LBL_REPORT_SEND_EMAIL_CC']."</em></a></li>
			<li><a href='javascript: void(0);' id='bcc_Tab' onClick='controllerReportEdit.manageEditionTabs(\"emailTabs\", \"bcc\", \"emailPanel\");'><em>".$mod_strings['LBL_REPORT_SEND_EMAIL_BCC']."</em></a></li>
		</ul>
	...
	<ul id="reportTabs" class="yui-nav">
		<li class="selected"><a id="mainInfo_Tab" href="javascript: void(0);" onClick="controllerReportEdit.manageEditionTabs(\'reportTabs\', \'mainInfo\', \'reportPanel\');"><em>'.$mod_strings ['LBL_REPORT_BASIC_INFO'].'</em></a></li>';
			
		$returnedHtml .= '
		<li class="'.($isRemoteExecuted ? 'disabled' : '').'"><a id="fieldsFilters_Tab" href="javascript: void(0);" onClick="controllerReportEdit.manageEditionTabs(\'reportTabs\', \'fieldsFilters\', \'reportPanel\');"><em>'.$mod_strings ['LBL_REPORT_FIELDS_FILTERS'].'</em></a></li>
		<li class="'.($extraParams['hasCharts'] ? '' : 'disabled').'"><a id="charts_Tab" href="javascript: void(0);" onClick="controllerReportEdit.manageEditionTabs(\'reportTabs\', \'charts\', \'reportPanel\');"><em>'.$mod_strings ['LBL_REPORT_CHARTS_TITLE'].'</em></a></li>
		<li class="'.($isScheduled ? '' : 'disabled').'"><a id="scheduledDiv_Tab" href="javascript: void(0);" onClick="controllerReportEdit.manageEditionTabs(\'reportTabs\', \'scheduledDiv\', \'reportPanel\');"><em>'.$mod_strings ['LBL_REPORT_SCHEDULED_TASKS'].'</em></a></li>
		<li class="'.($isNonExecutable ? 'disabled' : '').'"><a id="distributionList_Tab" href="javascript: void(0);" onClick="controllerReportEdit.manageEditionTabs(\'reportTabs\', \'distributionList\', \'reportPanel\'); }"><em>'.$mod_strings ['LBL_REPORT_DISTRIBUTION_LIST'].'</em></a></li>
	';
		
		$returnedHtml .= (asol_CommonUtils::isDomainsInstalled() ? '<li><a id="domainPublishing_Tab" href="javascript: void(0);" onClick="controllerReportEdit.manageEditionTabs(\'reportTabs\', \'domainPublishing\', \'reportPanel\'); }"><em>'.$mod_strings ['LBL_ASOL_DOMAINS_PUBLISH_FEATURE_PANEL'].'</em></a></li>' : '');
