var asolCalendar= (function($) {
	...
	var asolCalendarOnShow=function(embedGregorian,containerId,inputId){
		...
		$("a[class^='jd']").click(function(event){
			//Get event source and the selected JD
			...
			//Date must be created with USA format as String (Edge fucks you).
			var auxDate=dateConverted[1] + " " + dateConverted[0] + ", " + dateConverted[2];
			dateConverted=new Date(auxDate);
	...
	var initialize=function(containerId,inputID,editValue,inputClass,forceType){
		...
		//Initialize only if calendar picker does not have is-calendarsPicker class. This class is added when initialized. If more than one initialization is done for the same input, calendar icons will not be shown.
		if (datePickerInput.length > 0) {
			var datePickerInputClasses=datePickerInput.attr('class');
			if((datePickerInputClasses != null) && (datePickerInputClasses.indexOf("is-calendarsPicker") == -1)) {
				switch(forceType ? forceType : context.type){
					case 'persian':
						switch(context.language){
							case 'fa_ir':
								...
								if(!hasEditValue){
									//Pick what the input really has, if it has something.
									...
									if("undefined" != typeof editValue && editValue.length > 0){
										...
										//Date must be created with USA format as String (Edge fucks you).
										var auxDate=editValue[1] + " " + editValue[0] + ", " + editValue[2];
										editValue=new Date(auxDate);
										editValue=editValue.format(context.format);
									}
								}
								//Will need to convert dateVal to the other calendar (gregorian to persian) and if it applies, convert the digits to the corresponding characters.
			    				...
			    				datePickerInput.calendarsPicker({
									...
									showTrigger:'<i class=\"icn-calendar clickable calendarIcon '+inputClass+' trigger\"></i>',
				    			...
				    			$(containerRef).find("input[id='"+inputID+"_date']").siblings('.'+inputClass+'.trigger').after("<i class=\"icn-cancel clickable calendarIcon "+inputClass+" cancel\"></i>");
					...
					default:
						...
						//Dont need to convert from persian to gregorian because we are already at gregorian
