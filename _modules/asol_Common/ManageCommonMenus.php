...
$commonMenusHtml = '
	...
	<link href="modules/asol_Common/include/client/css/asolicons.css?version='.str_replace('.', '', asol_CommonUtils::$common_version).'" type="text/css" rel="stylesheet">
	...
	<script type="text/javascript">
		commonMenuManagement.setLanguage({
			...
			"close": "'.translate("LBL_COMMON_CANCEL", "asol_Common").'",
...
foreach($acl_modules as $module=>$ACLInfo) {
		...
		foreach ($common_menu_configuration["menus"] as $currentMenu) {
			...
			$commonCurrentModuleHtml .= '
						...
						<td width="10%" align="right">
							<i class="roles asol_icon icn-user" style="'.(isset($roles) ? 'color: #ead306;': '').'" onclick="commonMenuManagement.menuItemRoles(this)" config="'.(isset($roles) ? $roles: '{}').'" title="'.$mod_strings['LBL_COMMON_MENU_MANAGEMENT_ROLES'].'"></i>'	
							.($properties != '' ? '<i class="properties asol_icon icn-filter" style="'.(isset($propertiesConfig) ? 'color: #7f20e1;' : '').'" onclick="menuItemProperties(this);" config="'.(isset($propertiesConfig) ? $propertiesConfig: '[]').'" title="'.$mod_strings['LBL_COMMON_MENU_MANAGEMENT_VISIBILITY'].'"></i>' : '').
							'<i class="asol_icon icn-trash" onclick="if (confirm(\''.$mod_strings['LBL_COMMON_MENU_DELETE_ALERT'].'\')) { $(this).closest(\'.asolCommonMenuRow\').remove(); }"></i>
						</td>
					</tr>
				';
	...
	$commonMenusHtml .= '
									<i onClick="commonMenuManagement.saveMenuItems(\''.$module.'\')" class="asol_icon icn-floppy"></i>
									<i onClick="commonMenuManagement.addMenuItem(\''.$module.'\')" class="asol_icon icn-plus-circled"></i>
								</span>
