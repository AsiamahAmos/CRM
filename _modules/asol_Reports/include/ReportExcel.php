...
function generateCsv($fileConfig, $reportName, $headers, $resultset, $resultsetNoFormat, $headersTotals, $totals, $subTotals, $isDetailedReport, $isGroupedReport, $hasGroupedTotalBelowColumn, $returnData, $rowIndex, $columnsDataTypes, $columnsDataVisible, $removeLineBreaks=false, $removeTitles=false, $removeHeaders=false, $removeQuotes=false, $removeTotals=false) {
	...
	$exportCsvDelimiter = (!empty($fileConfig['separator']) ? $fileConfig['separator'] : (isset($sugar_config["asolReportsCsvDelimiter"]) ? $sugar_config["asolReportsCsvDelimiter"] : ","));
	...
	if (!$removeHeaders) {
		...
		foreach ($headers as $ckey) {
			...
			if (!in_array($dataVisible, array('html', 'internal'))) {
				$columns .= $ckey.$exportCsvDelimiter;
			}
		}
			...
			foreach ($headers as $cKey) {
				...
				if (!in_array($dataVisible, array('html', 'internal'))) {
					foreach ($totals as $total) {
						$rowSet .= ($removeQuotes ? (isset($total[$cKey]) ? $total[$cKey] : "").$exportCsvDelimiter : "\"".(isset($total[$cKey]) ? $total[$cKey] : "")."\"".$exportCsvDelimiter);
					}
				}
			}
			...
			if (!empty($subTotals) && !$removeTotals) {
				...
				if ($isGroupedReport && $hasGroupedTotalBelowColumn) {
					...
					foreach ($headers as $ckey) {
						$dataVisible = $columnsDataVisible[$ckey];
						if (!in_array($dataVisible, array('html', 'internal'))) {
							$rowTotals .= ($removeQuotes ? (isset($subTotals[$key][$ckey]) ? $subTotals[$key][$ckey] : '').$exportCsvDelimiter : "\"".(isset($subTotals[$key][$ckey]) ? $subTotals[$key][$column] : '')."\"".$exportCsvDelimiter);
						}
					}
...
function generateXls($fileConfig, $reportName, $headers, $resultset, $resultsetNoFormat, $headersTotals, $totals, $subTotals, $isDetailedReport, $isGroupedReport, $hasGroupedTotalBelowColumn, $returnData, $rowIndex, $columnsDataTypes, $columnsDataVisible, $removeLineBreaks=false, $removeTitles=false, $removeHeaders=false, $removeQuotes=false, $removeTotals=false){
	...
	require_once('modules/asol_Reports/include/server/libraries/PHPExcel/PHPExcel.php');
	//require_once('modules/asol_Reports/include/server/libraries/PHPExcel/PHPExcel/Writer/Excel2007.php');
	require_once('modules/asol_Reports/include/server/libraries/PHPExcel/PHPExcel/IOFactory.php');
	...
	if (!$removeHeaders) {
		...
		foreach ($headers as $ckey) {
			$dataVisible = $columnsDataVisible[$ckey];
			if (!in_array($dataVisible, array('html', 'internal'))) {
				$objPHPExcel->getActiveSheet()->SetCellValue($currentLetter.$currentNumber, $ckey);
				$currentLetter++;
			}
		}
			...
			foreach ($headers as $ckey) {
				...
				if (!in_array($dataVisible, array('html', 'internal'))) {
					foreach ($totals as $total) {
						...
						$value =  preg_replace("/[\n|\r|\n\r]/i", ' ', html_entity_decode((isset($total[$ckey]) ? $total[$ckey] : ""), ENT_QUOTES, 'UTF-8'));
			...
			if (!empty($subTotals) && !$removeTotals) {
				...
				if ($isGroupedReport && $hasGroupedTotalBelowColumn) {
					...
					foreach ($headers as $ckey) {
						$dataVisible = $columnsDataVisible[$ckey];
						if (!in_array($dataVisible, array('html', 'internal'))) {
							$objPHPExcel->getActiveSheet()->setCellValue($currentLetter.$currentNumber, (isset($subTotals[$key][$ckey]) ? $subTotals[$key][$ckey] : ''));
							$currentLetter++;
						}
					}
