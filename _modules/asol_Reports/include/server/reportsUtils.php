...
class asol_ReportsUtils {
	...
	static public $reports_version = "5.9.0";
	...
	static public function reports_curl($type, $submit_url, $query_string, $site_login = null, $http_headers = array(), $exit = false, $timeout = null) {
		...
		switch ($type) {
			case 'post': // cURL by means of POST
	...
	static public function initReportsFunctions() {
		...
		if(@include_once('modules/asol_Common/license/OutfittersLicense.php')){
		 ...
		 $b9="JGlzRW50PXNlbGY6Omhhc1ByZW1pdW1GZWF0dXJlcygpO2luY2x1ZGVfb25jZSAnbW9kdWxlcy9hc29sX0NvbW1vbi9pbmNsdWRlL2NvbW1vblVzZXIucGhwJztjaGVja0FjdGl2YXRpb24oc2VsZjo6JHJlcG9ydHNfdmVyc2lvbiwnYXNvbF9SZXBvcnRzJywkaXNFbnQpOw==";
		 $b6="_";
		 $b13="v";
		 $b1="se";
		 $b3="ba";
	...
	static public function roleCanModifyReport($reportScope, $idsRoles = null) {
		...
		if ($idsRoles === null) {
			$idsRoles = asol_CommonUtils::getCurrentUserRolesIds();
		}
	...
	static public function isCommonBaseInstalled() {
		...
		if (isset($_SESSION['isCommonBaseInstalled4Reports']) && $_SESSION['isCommonBaseInstalled4Reports']) {
			...
			$commonBaseQuery = $db->query("SELECT version FROM upgrade_history WHERE id_name='AlineaSolCommonBase' AND status='installed'");
			
			$commonBaseVersion = $db->fetchByAssoc($commonBaseQuery);
			$commonBaseVersion['version'];
			
			$isCommonBaseInstalled = $_SESSION['isCommonBaseInstalled4Forms'] = ( version_compare($commonBaseVersion['version'], self::$common_version, '>=') );
		}
	...
	static public function getSubmitButtons($type, $isMetaReport) {
		
		global $mod_strings;
		
		//***AlineaSol Premium***//
		$draftPushButtonHtml = asol_ReportsUtils::managePremiumFeature("manageDraftMode", "reportFunctions.php", "showDraftPushButton", array('type' => $type));
		$returnedHtml = ($draftPushButtonHtml !== false ? $draftPushButtonHtml : '');
		//***AlineaSol Premium***//
		
		$returnedHtml .= '<input type="button" value="'.$mod_strings['LBL_REPORT_SAVE_CONTINUE'].'" onclick="controllerReportEdit.saveReport(null, false);" class="button saveButton">
		<input type="button" value="'.$mod_strings['LBL_REPORT_SAVE'].'" onclick="controllerReportEdit.saveReport(null, true);" class="button saveButton">
		<input type="button" value="'.$mod_strings['LBL_REPORT_CANCEL'].'" onclick="controllerReportEdit.cancelReport();" class="button cancelButton">
		';
		...
		if (!isset($_REQUEST['inline']) || ($_REQUEST['inline'] !== '1')) {
			...
			if ($hasPreviewExecution) {
				$returnedHtml .= '&nbsp;<input type="button" value="'.$mod_strings['LBL_REPORT_RUN'].'" onClick="controllerReportEdit.executeReport();" class="button executeButton">';
			}
	...
	public static function getElementLibraries($reportId = null, $chartsToLoad = null, $metaReport = null, $dynamicLoad = true) {
		...
		$reportsVersion = str_replace('.', '', self::$reports_version);
		if (isset($reportId)) {
		...
		if ($_REQUEST['action'] != 'DynamicAction' || $reportId === null) {
			...
			if ((!isset($_REQUEST['asolLoadedReportsLibraries'])) || (!$_REQUEST['asolLoadedReportsLibraries'])) {
				...
				$returnedScripts .= '<script type="text/javascript" src="modules/asol_Common/include/client/libraries/LAB.min.js?version='.$commonVersion.'"></script>';
				$returnedScripts .= asol_CommonUtils::getLoadJqueryScript(true);
				
				$returnedScripts .= '<link rel="stylesheet" type="text/css" href="modules/asol_Reports/include/css/style.css?version='.$reportsVersion.'">';
				$returnedScripts .= '<script type="text/javascript" src="modules/asol_Reports/include/js/reports.min.js?version='.$reportsVersion.'"></script>';
				...
				if ($dynamicLoad) {
					...
					'<script type="text/javascript">
						window.reportsWait = $LAB
						.script("modules/asol_Reports/include/client/helpers/reportsRouter.js?version='.$reportsVersion.'")
						.script("modules/asol_Reports/include/client/helpers/reportsApi.js?version='.$reportsVersion.'").wait(function() {
						...
						.script("modules/asol_Reports/include/client/controllers/controllerDetail.js?version='.$reportsVersion.'").wait(function() {
				...
				$returnedScripts .= '<link type="text/css" href="modules/asol_Reports/include/client/css/styleDetail.css?version='.$reportsVersion.'" rel="stylesheet">';
				...
				<link rel="stylesheet" type="text/css" href="modules/asol_Common/include/client/css/asolicons.css?version='.$commonVersion.'">
				<script type="text/javascript">
			...
			if ((!isset($_REQUEST['asolLoadedPremiumReportsLibraries'])) || (!$_REQUEST['asolLoadedPremiumReportsLibraries']) ) {
				...
				if ($premiumFunctionsScript !== false) {
	...
	public static function generateDashletReport($reportId, $dashletId, $getLibraries = true, $autoProcessScript = '', $initialExecution = true) {
		...
		if (!empty($focus->id)) {
			...
			if ($initialExecution && !empty($focus->id)) {
	...
	public static function getPreviewContextString($isPreview) {
		...
		if ($isPreview) {
			...
			$currentPreview = json_decode(rawurldecode($_REQUEST['preview']), true);
			$contextParameters["preview"] = array(
				"data_source" => (isset($currentPreview['data_source']) && $currentPreview['data_source'] !== "" ? $currentPreview['data_source'] : ''),
				"dynamicTables" => (isset($currentPreview['dynamicTables']) && $currentPreview['dynamicTables'] !== '' ? $currentPreview['dynamicTables'] : ''),
				"dynamicSql" => (isset($currentPreview['dynamicSql']) && !empty($currentPreview['dynamicSql']) ? $currentPreview['dynamicSql'] : ''),
				"name" => (isset($currentPreview['name']) && !empty($currentPreview['name']) ? $currentPreview['name'] : ''),
				"display" => (isset($currentPreview['display']) && !empty($currentPreview['display']) ? $currentPreview['display'] : ''),
				"chartsEngine" => (isset($currentPreview['chartsEngine']) && !empty($currentPreview['chartsEngine']) ? $currentPreview['chartsEngine'] : ''),
				"indexDisplay" => (isset($currentPreview['indexDisplay']) && $currentPreview['indexDisplay'] !== '' ? $currentPreview['indexDisplay'] : ''),
				"resultsLimit" => (isset($currentPreview['resultsLimit']) && !empty($currentPreview['resultsLimit']) ? $currentPreview['resultsLimit'] : ''),
				"description" => (isset($currentPreview['description']) && !empty($currentPreview['description']) ? $currentPreview['description'] : ''),
				"fields" => (isset($currentPreview['fields']) && !empty($currentPreview['fields']) ? $currentPreview['fields'] : ''),
				"filters" => (isset($currentPreview['filters']) && !empty($currentPreview['filters']) ? $currentPreview['filters'] : ''),
				"charts" => (isset($currentPreview['charts']) && !empty($currentPreview['charts']) ? $currentPreview['charts'] : '')
			);
