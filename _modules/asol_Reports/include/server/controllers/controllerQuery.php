...
class asol_ControllerQuery {
	...
	static public function getSqlJoinQuery(& $fieldValues, & $filterValues, $hasDeletedUsage, $resultsLimit, $reportModule, $reportTable, $auditedReport, $alternativeDb, & $fieldsByRef, $aliasSuffix = '', & $domainField = null) {
		...
		foreach ($fieldValues['tables'][0]['data'] as $index => & $currentValues) {
			...
			//**************************************//
			$isFieldVisible = (in_array(strtolower($currentValues['visible']), array("yes", "html", "internal")) && self::propertyApplies($currentValues['visibilityConfig']) && self::roleAclApplies($currentValues['roles']) ); 
			...
			$currentDoJoin = ($commonDoJoin || ($isFieldVisible || $fieldSqlVarsInFilters));
			$currentDoCountJoin = ($commonDoJoin || (($isFieldVisible && $fieldHasFunction) || $fieldOrderedWithLimit));
			...
			foreach ($keyExploded as $keyDepth => $currentLink) {
				...
				if ($isNonCrmDatabase) {
					...
					$parentRelationshipID = ($keyDepth >= 1 ? $parentLinkExploded[1] : null);
					$relationshipID = null;
					$joinTable = null;
		...
		foreach ($joinVariables as $initKey=>$mainJoinFields) {
			...
			foreach ($mainJoinFields as $mainJoinIndex=>$secondJoinFields) {
				...
				foreach ($secondJoinFields as $joinKey=>$joinVar) {
					...
					$parentRelationshipID = $joinVar['parentRelationshipID'];
					$relationshipID = $joinVar['relationshipID'];
					...
					//************************//
					if ($mainJoinIndex > 0) {
						if (!empty($parentRelationshipID) && isset($aliasIndexTable[$initKey][$mainJoinIndex-1][$parentKey][$parentRelationshipID])) {
							$parentAliasIndex = $aliasIndexTable[$initKey][$mainJoinIndex-1][$parentKey][$parentRelationshipID][$mainTable].$aliasSuffix;
						} else {
							$parentAliasIndex = $aliasIndexTable[$initKey][$mainJoinIndex-1][$parentKey][$mainTable].$aliasSuffix;
						}
					} else {
						$parentAliasIndex = $aliasSuffix;
					}
					...
					if (isset($relationshipID)) {
						$aliasIndexTable[$initKey][$mainJoinIndex][$mainKey][$relationshipID][$relatedTable] = $currentAliasIndex;
					} else {
		...
		//*******************************************************//
		$sqlCurrentJoinQuery = $sqlCurrentCountJoinQuery = ' LEFT JOIN '.$reportTable.'_cstm '.$reportTable.'_cstm'.$aliasSuffix.' ON ('.$reportTable.$aliasSuffix.'.id='.$reportTable.'_cstm'.$aliasSuffix.'.id_c)';
	...
	static private function resetValuesWithJoinAlias(& $storedValues, $aliasIndexTable, $reportTable, $auditedReport, & $fieldsByRef, $aliasSuffix = '', $areFields = true) {
		...
		foreach ($storedValues as & $currentValues) {
			...
			$initRelation = (isset($initKeyExploded[1]) ? $initKeyExploded[1] : null);
			$mainJoinIndex = (count($keyExploded) - 1);
			$mainKey = $currentKeyExploded[0];
			$mainRelation = (isset($currentKeyExploded[1]) ? $currentKeyExploded[1] : null);
			...
			if ($isCustomField) {
				$currentValues['field'] = $fieldExploded[0].$aliasSuffix.'.'.$fieldExploded[1];
			}
			
			$relatedRealTable = ($isCustomField ? substr($relatedTable, 0, -5) : $relatedTable);

			if ($currentValues['isRelated']) {	
				if (isset($aliasIndexTable[$initKey][$mainJoinIndex][$mainKey][$mainRelation])) {
					$usedIndex = $aliasIndexTable[$initKey][$mainJoinIndex][$mainKey][$mainRelation][$relatedRealTable];
				} else {
	...
	static public function getAvailableColumns($data_source, & $selectedFields, $predefinedTemplates, $overrideSort, $displayTotals, $displaySubtotals, $currentTable, $weekStart, $quarterMonth, $hourOffset, $joinIndexes, $aliasSuffix = '') {
		
		$availableColumns = array();
		$availableTotals = array();
		...
		foreach ($selectedFields['tables'][0]['data'] as $index => & $currentValues) {
			...
			$currentValues['originalFieldName'] = $currentValues['field'];
			if ($data_source['type'] === '1') {
				$currentValues['notModifiedFieldName'] = $currentValues['field'];
			}
			
			//***AlineaSol Premium***//
			$currentValues['untranslatedAlias'] = trim($currentValues['alias']);
			$returnedAlias = asol_ReportsUtils::managePremiumFeature("multiLanguageReport", "reportFunctions.php", "getMultiLanguageLabel", array('label' => $currentValues));
			$currentValues['alias'] = ($returnedAlias !== false ? $returnedAlias : trim($currentValues['alias']));
			//***AlineaSol Premium***//
			
			$currentValues['enumLabels'] = (in_array($currentValues['enumOperator'], array('options', 'function')) ? asol_Reports::getEnumLabels($data_source, $currentValues['field'], $currentValues['enumOperator'], $currentValues['enumReference']) : array());
			...
			//***AlineaSol Premium***//
			$extraParams = array('currentValues' => $currentValues, 'currentConfig' => $selectedFields['tables'][0]['config'], 'fieldValues' => $selectedFields);
			if ($data_source['type'] === '0') {
				$response = asol_ReportsUtils::managePremiumFeature("getSqlForTargetField", "reportFunctions.php", "getSqlForTargetField", $extraParams);
			} else if ($data_source['type'] === '1') {
				$response = asol_ReportsUtils::managePremiumFeature("getPhpForTargetField", "reportFunctions.php", "getPhpForTargetField", $extraParams);
			}
			$currentValues = ($response !== false ? $response : $currentValues);
			//***AlineaSol Premium***//
			...
			if ($data_source['type'] == '0') {
				...
				$sqlFunction = self::replaceReportsFieldVars($currentValues['sql'], $currentTable, $joinIndexes, $data_source['value']['audit'], $data_source['value']['database']);
				
				if (($data_source['value']['database'] == '-1') && ($data_source['value']['audit'] == '1')) {
					$table = ((count(explode(".", $currentValues['field']))) == 1 ) ? $currentTable."_audit.".$aliasSuffix : "";
				} else {
					$table = ((count(explode(".", $currentValues['field']))) == 1 ) ? $currentTable.$aliasSuffix."." : "";
				}
				$currentValues['field'] = $table.$currentValues['field'];
				
				//***No Aggregated Function***//
				if (in_array($currentValues['function'], array("0", "undefined"))) {
					if (!empty($sqlFunction)) {
						self::updateMySqlFunctionField($sqlFunction, null, $currentValues['field']);
					}
				//***Aggregated Function***//
				} else {
					$distintQuery = '';
					$dbKey = ($data_source['value']['database'] === '-1' ? 'crm' : 'ext'.$data_source['value']['database']);
					if (($currentValues['function'] === 'COUNT') && (!$currentValues['isRelated']) && (isset($_SESSION['asolReportsTableIndexes'][$dbKey][$currentTable][$currentValues['originalFieldName']]))) {
						$distintQuery = 'DISTINCT ';
					}
					
					$currentValues['field'] = $currentValues['function']."(".$distintQuery.$currentValues['field'].")";
			...
			if ($data_source['type'] === '0' && $currentValues['grouping'] !== '0') {
				...
				if ($whereFunction !== null) {
					self::updateMySqlFunctionField($whereFunction, null, $currentValues['field']);
				}
			...
			if (in_array($currentValues['grouping'], array("Detail", "Minute Detail", "Quarter Hour Detail", "Hour Detail", "Day Detail", "DoW Detail", "WoY Detail", "Month Detail", "Natural Quarter Detail", "Fiscal Quarter Detail", "Natural Year Detail", "Fiscal Year Detail"))) {
				...
				$details[] = array(
					...
					'treeValues' =>	$currentValues['treeValues'],
				);
			}
		...
		foreach ($selectedFields['tables'][0]['data'] as $index => & $currentValues) {
			...
			$isFieldVisible = (in_array(strtolower($currentValues['visible']), array("yes", "html", "internal")) && self::propertyApplies($currentValues['visibilityConfig']) && self::roleAclApplies($currentValues['roles']));
			$hasFunction = (!in_array($currentValues['function'], array("0", "undefined")));
			
			if ($isFieldVisible) {
				
				if ($hasFunction) {
					if ($hasGrouped && $displaySubtotals) {
						$availableColumns[$currentValues['alias']] = array(
							'u' => ($data_source['type'] == '0' ? $currentValues['untranslatedAlias'] : $currentValues['alias']),
							'o' => ($data_source['type'] == '0' ? $currentValues['originalFieldName'] : $currentValues['field']),
							'n' => ($data_source['type'] == '0' ? $currentValues['notModifiedFieldName'] : $currentValues['field']),
							'r' => $currentValues['fieldReference'],
							'i' => $currentValues['index']
						);
						$availableFields[] = $currentValues;
					}
					if ($displayTotals) {
						$availableTotals[] = $currentValues;
					}
					$hasTotals = true;
				} else {
					$availableColumns[$currentValues['alias']] = array(
			...
			if (!empty($overrideSort['field'])) {
				if (($currentValues['originalFieldName'] == $overrideSort['field']) && ($currentValues['index'] == $overrideSort['index'])) {
					$availableOrders[] = array(
						'field' => $currentValues['field'],
						'original' => $currentValues['originalFieldName'],
						'alias' => $currentValues['alias'],
						'index' => $overrideSort['index'],
						'type' => $currentValues['type'],
						'order' => '1',
						'direction' => (empty($overrideSort['direction']) ? "ASC" : $overrideSort['direction'])
					);
				}
			} else {
				$orderByNotGroupedFunction = (!$hasGrouped && ($currentValues['function'] !== "0"));
				if (($currentValues['sortDirection'] !== "0") && (!$orderByNotGroupedFunction)) {
					$availableOrders[] = array(
						...
						'direction' => $currentValues['sortDirection']
					);
				}
			}
	...
	static public function getSqlSelectQuery(& $fieldValues, & $chartInfo, $currentConfig, $data_source, $reportTable, $hourOffset, $quarterMonth, $weekStart, $displayTotals, $displaySubtotals, $fieldsByRef, $filtersByRef, $aliasIndexTable = array(), $aliasSuffix = '') {
		...
		if (isset($currentConfig['addDistinctStatement']) && $currentConfig['addDistinctStatement']) {
			$sqlSelect .= "DISTINCT ";
			$sqlTotals .= "DISTINCT ";
			$sqlTotalsC .= "DISTINCT ";
		}
		...
		foreach ($fieldValues['tables'][0]['data'] as $i => & $currentValues) {
			...
			$isFieldVisible = (in_array(strtolower($currentValues['visible']), array("yes", "html", "internal")) && self::propertyApplies($currentValues['visibilityConfig']) && self::roleAclApplies($currentValues['roles']));
			$sqlFunction = self::replaceReportsFieldVars($currentValues['sql'], $reportTable, $aliasIndexTable, $data_source['value']['audit'], $data_source['value']['database']);
	...
	static public function generateSqlWhere(& $fieldValues, $filterValues, $reportTable, $hourOffset, $currentConfig, $fieldsByRef, & $filtersByRef, $avoidTrim, $avoidEmptyFilters, $avoidAutocomplete, $aliasSuffix = '') {

		if (!empty($filterValues)) {

			foreach ($filterValues as & $filter) {
	
				if (($filter['apply'] === '0') || !self::propertyApplies($filter['visibilityConfig']['properties']))
					continue;
				
				$filter['field'] = ((count(explode(".", $filter['field']))) == 1  ? $reportTable.$aliasSuffix.".".$filter['field'] : $filter['field']);
				
				$appliesFilter = self::modifyFilteringValues($filter, $operator1, $operator2, $currentConfig, $hourOffset, $avoidTrim, $avoidAutocomplete);
				self::populateFilteringValues($filter, $operator1, $operator2, $appliesFilter, $filtersByRef, $avoidEmptyFilters, $avoidAutocomplete);
	...
	static public function modifyFilteringValues(& $filter, & $operator1, & $operator2, $currentConfig, $hourOffset, $avoidTrim, $avoidAutocomplete) {

		global $current_user, $timedate;
		
		$quarterMonth = $currentConfig['quarter_month'];
		...
		$filter['parameters']['first'][0] = asol_CommonUtils::replaceStaticGlobalVars($filter['parameters']['first'][0]);
		$filter['parameters']['second'][0] = asol_CommonUtils::replaceStaticGlobalVars($filter['parameters']['second'][0]);
		$filter['parameters']['third'][0] = asol_CommonUtils::replaceStaticGlobalVars($filter['parameters']['third'][0]);
		...
		switch ($filter['operator']) {
			...
			case "my items":
				...
				if (!empty($filter['parameters']['first'][0]) && $filter['parameters']['first'][0] !== 'true' ) $operator1 = "!=";
				
				$filter['parameters']['first'][0] = $current_user->id;
	...
	static private function populateFilteringValues($filter, $operator1, $operator2, $appliesFilter, & $filtersByRef, $avoidEmptyFilters, $avoidAutocomplete) {
		...
		if ($appliesFilter || $hasParenthesis) {
			...
			$hasEmptyParameters = (in_array($filter['parameters']['first'][0], array('', 'null'))) && (in_array($filter['parameters']['second'][0], array('', 'null'))) && (in_array($filter['parameters']['third'][0], array('', 'null'))) || ( $filter['operator'] === 'my items' && $operator1 === '!=' );			
			if ($avoidEmptyFilters && $hasEmptyParameters) {
				
				$currentSqlWhere .= 'true';
		...
		if (!empty($filtersByRef)) {
			...
			foreach ($filtersByRef as $currentFilter) {
				...
				if ($currentFilter['isWhereFilter']) {
					...
					$hasWhereFilters = ($hasWhereFilters || true);
			...
			} else if ($isNonCrmDatabase && !$hasWhereFilters) {
				$sqlWhere = "";
			} else {
	...
	static public function modifySqlWhereForSecurityGroups(& $sqlWhere, $securityReportModule, $securityReportTable, $current_user) {
		...
		} else if (is_object($bean) && $bean->bean_implements('ACL') && ACLController::requireSecurityGroup($securityReportModule, 'list')) {
	...
	static public function getSqlGroupByQuery($fieldValues, $reportTable, $details, $groups, & $filtersByRef) {

		$sqlGroup = "";
		$sqlChartGroup = "";
		$hasDetail = (count($details) > 0);
		$hasGrouped = (count($groups) > 0);
		$hasFunctionField = false;
		$hasAnyGroupVisible = false;
		...
		foreach ($fieldValues['tables'][0]['data'] as $currentValues) {
			...
			//*******************************//
			if (in_array($currentValues['grouping'], array("Grouped", "Minute Grouped", "Quarter Hour Grouped", "Hour Grouped", "Day Grouped", "DoW Grouped", "WoY Grouped", "Month Grouped", "Natural Quarter Grouped", "Fiscal Quarter Grouped", "Natural Year Grouped", "Fiscal Year Grouped"))) {
				$hasAnyGroupVisible = ($hasAnyGroupVisible || (strtolower($currentValues['visible']) == 'yes'));
			}
		...
		$returnedArray = array (
			"hasAnyGroupVisible" => $hasAnyGroupVisible,
	...
	static public function getSqlOrderByQuery($fieldValues, $reportTable, $alternativeDb, $orders, $maxAllowedNotIndexedOrderBy, $fieldsByRef) {
		...
		foreach ($fieldValues['tables'][0]['data'] as $currentValues) {
			...
			foreach ($orders as & $currentOrder) {
				if (($currentValues['originalFieldName'] == $currentOrder['original']) && ($currentValues['index'] == $currentOrder['index'])) {
					if ($currentValues['format']['type'] == 'button' && isset($currentValues['format']['extra']['label']['text'])) {
						preg_match("/^%{(.*?)}/", $currentValues['format']['extra']['label']['text'], $fieldRef);
						if (count($fieldRef) > 1) {
							$fieldRef = $fieldRef[1];
							$currentOrder['field'] = $fieldsByRef[$fieldRef]['field'];
						}
					}
				}
			}
			
			if ($maxAllowedNotIndexedOrderBy && $currentValues['sortDirection'] !== "0") {
		...
		if (count($orders) > 0) {
			...
			$sqlOrder .= " ORDER BY ";
			foreach ($orders as $currentOrder) {
				$sqlOrder .= $currentOrder['field']." ".$currentOrder['direction']." ,";
			}
	...
	static public function getExternalTablePrimaryKey($database, $table) {
		
		if (empty($table)) {
			return null;
		}
		
		$alternativeDb = ($database >= 0 ? $database : false);
		$dbKey = ($alternativeDb === false ? 'crm' : 'ext'.$alternativeDb);
		
		if (!isset($_SESSION['asolReportsTablePrimaryKey'][$dbKey][$table])) {
			
			$mainPK = asol_Reports::getSelectionResults("SHOW KEYS FROM ".$table." WHERE Key_name = 'PRIMARY'", null, true, $alternativeDb);
			$primaryKey = $mainPK[0]['Column_name'];
			
			$_SESSION['asolReportsTablePrimaryKey'][$dbKey][$table] = $primaryKey;
			
		} else {
			
			$primaryKey = $_SESSION['asolReportsTablePrimaryKey'][$dbKey][$table];
	...
	static public function formatSubGroup($reportId, $dashletId, $subGroup, $subGroupInfo, $userTZ, $gmtDates) {
		...
		if (in_array($formatType, array("enum", "radioenum"))) {
			...
			$extraParams = array(
				...
				'currentEnum' => $subGroupInfo['treeValues'],
			...
			$subGroup = ($returnedPremiumTree !== false ? $returnedPremiumTree['value'] : $subGroup);
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
		...
		switch ($fieldInfo['grouping']) {
			...
			case "Day Detail":
				...
				if ((!$timedate->check_matching_format($subGroup, $userDateFormat)) && ($subGroup != "")){
					...
					if ($calendarType == 'persian') {
						$subGroup = asol_CommonUtils::dateToPersian($subGroup, false, $userDateFormat);
					} else {
	...
	static private function formatResultSetRows($reportId, $dashletId, $resulsetFields, & $rowValues, $userDateFormat, $userDateTimeFormat, $userTZ, $gmtDates, $isGroupedReport, $auditedReport, $auditedAppliedFields, $auditedFieldType, $predefinedTemplates, $referenceAlias, & $availableMassiveButtons) {
		...
		foreach ($resulsetFields as & $fieldDef) {
			...
			$rowValues[$fieldDef['alias']] = self::visibilityFieldsApplies( $fieldDef['visibilityConfig'], $fieldDef, $rowValues[$fieldDef['alias']] );
			...
			if (!$isTotal) {
				...
				if (in_array($formatType, array("enum", "radioenum"))) {
					...
					if (($currentUnformattedValue != "") && (date('Y-m-d H:i:s', strtotime($currentUnformattedValue)) == $currentUnformattedValue)) {
						...
						if ($calendarType == 'persian') {
							$rowValues[$fieldDef['alias']] = asol_CommonUtils::dateToPersian($rowValues[$fieldDef['alias']], true, $userDateTimeFormat);
						} else {
					...
					$currentUnformattedValue = intval($currentUnformattedValue);
	...
	static public function formatGroupedFields($resultset, $groups, $userDateFormat) {

		foreach ($resultset as & $currentRow) {
			foreach ($groups as $currentGroup) {
				$currentRow[$currentGroup['alias']] = self::applyGroupFormat($currentRow[$currentGroup['alias']], $currentGroup['grouping'], $userDateFormat);
				$currentRow[$currentGroup['alias']] = html_entity_decode($currentRow[$currentGroup['alias']]);				
			}
		}
	...
	static public function generateManuallySubTotals($resultset, $totals, $resultsLimit, $groupedTotals = false, $subTotalsLimit = array()) {
		
		if ($resultset !== null) {
			foreach ($resultset as $loop => & $currentSubtotal) {
				foreach ($totals as $oneTotal) {
					if ($groupedTotals) {
						$theGroup = $currentSubtotal['asol_grouping_field'];
						$subTotalsLimit[$theGroup][$loop][$oneTotal['alias']] = $currentSubtotal[$oneTotal['alias']];
					} else {
						$subTotalsLimit[$loop][$oneTotal['alias']] = $currentSubtotal[$oneTotal['alias']];
					}
				}
				if ($groupedTotals) {
					unset($currentSubtotal['asol_grouping_field']);
				}
			}
		}
	...
	static private function replaceReportsFieldVars($textWithVars, $tableAliasName, $aliasIndexTable, $auditedReport, $database) {
		...
		foreach ($beanItems as $beanItem) {
			...
			if (count($beanValues) == 2) {
				...
				if ($beanValues[0] == "bean") {
					...
					$beanField = $tableAliasName.($auditedReport == '1' ? "_audit" : "").".".$beanValues[1];
			...
			if (count($beanValues) == 2) {
				...
				foreach ($usedModules as $usedIndex=>$usedModule) {
					...
					if ($isNonCrmDatabase) {
						...
						$relatedKey = self::getExternalTablePrimaryKey($alternativeDb, $usedModule);
	...
	static public function propertyApplies($visibilityConfig) {
		...
		if (isset($visibilityConfig)) {
			...
			if (!isset($visibilityConfig['properties'])) {
				$properties = json_decode(urldecode($visibilityConfig), true);
			} else {
				$properties = $visibilityConfig['properties'];
			}
	...
	static private function visibilityFieldsApplies($visibilityConfig, $fieldDef, $value){
		
		$visibilityConfigFields = (isset($visibilityConfig['fields']) && $visibilityConfig['fields'] != '' ? json_decode(urldecode($visibilityConfig['fields']),true) : null );

		if(isset($visibilityConfigFields)){
			foreach ($visibilityConfigFields as $field){
				if($field['ref'] == $fieldDef['fieldReference'] && $field['val'] == $value ){
					return  null;
				}
			}
		}
	...
	static private function roleAclApplies($currentRoles) {
		
		if (empty($currentRoles)) {
			return true;
		}
		
		if (!isset($currentRoles['roles'])) {
			$currentRoles = array('roles' => $currentRoles);
		}
		
		$userRoles = asol_CommonUtils::getCurrentUserRolesIds();

		$rolesApplies = array_intersect($userRoles, $currentRoles['roles']);
		$rolesApplies = !empty($rolesApplies);
		
		$aclsApplies = true;
		
		if (!empty($currentRoles['acls']) && !$rolesApplies) {
			reset($currentRoles['acls']);
			$aclModule = key($currentRoles['acls']);
			foreach ($currentRoles['acls'][$aclModule] as $action) {
				if (!ACLController::checkAccess($aclModule, $action, true)) {
					$aclsApplies= false;
					break;
				}
			}
		...
		return ($rolesApplies|| empty($userRoles) || (empty($currentRoles['roles']))) && $aclsApplies;
