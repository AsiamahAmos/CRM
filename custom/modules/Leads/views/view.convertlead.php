...
class ViewConvertLead extends SugarView
{
        ...
        )
    {
        ...
        if (file_exists("custom/$this->fileName"))
        {
           ...
           // $this->meta_file ="custom/$this->data";
        }
    }
  ...
  public function load_client_since_date(){//load client since date on convert lead 
    //code for opportunities is also here.
    print('<script>var today = new Date();
    var dd = String(today.getDate()).padStart(2, \'0\');
    var mm = String(today.getMonth() + 1).padStart(2, \'0\'); //January is 0!
    var yyyy = today.getFullYear();
    today = mm + \'/\' + dd + \'/\' + yyyy;
    $("#Contactsclient_since_c").val(today);
    $("#Opportunitiessales_stage").val($("option[label = \'Closed Won\']").val());
   // setInterval(function(){
    $("#btn_Opportunitiesrelated_product_c").blur(function(){
       // alert("change");
      // if($("#Opportunitiesaos_products_id_c").val()!=""){
        var total = $("#Opportunitiesrelated_product_c").val();
        $(\'#Opportunitiesname\').val(total);
             //}
           });
       // },100);
    ///load the opportunity form automatically///
    // $("#convert_create_Opportunities").attr("value","true");
    // $("#newOpportunities").attr("checked","checked");
    // toggleDisplay(\'createOpportunities\');
    // if(typeof(addRemoveDropdownElement) == \'function\') addRemoveDropdownElement(\'Opportunities\');
     </script>');
  }

  public function create_banking_account(){
  
  print('<script>
  $(".").click(function(e){
  var fname = $("#Contactsfirst_name").val();
  var lname = $("#Contactslast_name").val();
  var wMonths = ["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"];
  var dat = $("#Contactsclient_since_c").val().split("/");
  var dob =  dat[1]+"-"+ wMonths[parseInt(dat[0])-1] + "-" +dat[2];
  var sex = ($("#Contactsgender_c").val()=="MALE")? "M":"F";
  alert(dob);
  var homeadd = $("#Contactsprimary_address_street").val();
  var postaladd = $("#Contactsprimary_address_postalcode").val();
  var subprod  =$("#Opportunitiesaos_products_id_c").val();
  var branch = $("#Contactsbran_branches_id_c").val();
  var personalphone = ($("#Contactsphone_work").val()=="") ? $("#Contactsphone_mobile").val():$("#Contactsphone_work").val();
  var isdat = $("#Contactsclient_since_c").val().split("/");
  var issuedate =  isdat[1]+"-"+ wMonths[parseInt(isdat[0])-1] + "-" +isdat[2];
   alert(issuedate);
  var occupation = $("#Contactsoccupation_c").val();
  var segment = ($("#account_name").val()=="")?$("#Accountsname").val():$("#account_name").val();
  var armcode = $("#Contactsassigned_user_id").val();
  //var terminal = $("").val();
  alert(armcode);
    var settings = {
        "async": true,
        "crossDomain": true,
        "url": "http://192.168.1.240:8181/ords/xapi/newacct/new",
        "method": "POST",
        "headers": {
          "FNAME": fname,
          "LNAME": lname,
          ...
          "Postman-Token": "694444c8-f480-4370-838d-251a2e010659,09fce922-9f83-4d22-90dd-93efd372f819",
          "Host": "192.168.1.240:8181",
      ...
      $.ajax(settings).done(function (response,textStatus, request) {
        ...
        console.log(request.getResponseHeader("created_acct_no"));
        var customer_id = request.getResponseHeader("created_customer_no");
        alert(request.getResponseHeader("created_customer_no"));
	...
	public function display()
    {
        ...
        $this->load_client_since_date();
        $this->create_banking_account();
    }
        ...
        )
    {
        ...
        foreach($beans as $bean)
        {
            ...
            $stmt = $db->query("SELECT contacts.*,contacts_cstm.* FROM contacts,contacts_cstm WHERE contacts.id = contacts_cstm.id_c AND contacts_cstm.id_c = '$bean->id' ");
            // $stmt = $db->query("SELECT * FROM contacts_cstm WHERE contacts_cstm.id_c = '$bean->id' ");
            $res = $db->fetchByAssoc($stmt);//was not used
            foreach ($stmt as $key => $value) {
             # code...
        $id_c = $value['id_c'];     
        $fname = $value['first_name']; $lname = $value['last_name'];
        $bd_arr = explode('-',$value['birthdate']);
        $month = array('JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC');
        $dob = $bd_arr[2].'-'.$month[$bd_arr[1]-1].'-'.$bd_arr[0];
        $homeadd = $value['primary_address_street']; $issuedate = $value['client_since_c'];$personalphone = ($value['phone_mobile'])?$value['phone_mobile']:$value['phone_work'];
        $postalcode = $value['primary_address_postalcode']; $gender = $value['gender_c']; $armcode = $value['assigned_user_id'];
        $branch =  $value['bran_branches_id_c']; $subprod = $_REQUEST['Opportunitiesaos_products_id_c'];$segment = $_REQUEST['account_name'];
          // echo "<pre><td>".$_REQUEST['Contactsfirst_name'];echo "</td></pre>";
           
                    $curl = curl_init();
                    
                    curl_setopt_array($curl, array(
                      CURLOPT_PORT => "8181",
                      CURLOPT_URL => "http://192.168.1.240:8181/ords/xapi/newacct/new",
                      ...
                      CURLOPT_HTTPHEADER => array(
                        ...
                        "Host: 192.168.1.240:8181",
                        "ISSUEAUTH: GHA",
                        "ISSUEDTE: 12-oct-2019",
                        "LNAME: $lname",
                        "NATIONALITY: GHA",
                        "OCCUPATION: RETAILER",
                        "PERSONLPHONE: $personalphone",
                        "POSTALADD: $postalcode",
                        "Postman-Token: 8bd71ad4-2095-461d-a11c-54f27335c537,1b6e5d57-48f1-41d5-bcd6-d35f20ad20b6",
                        "SEX: $gender",
                        "SUBPROD: $subprod",
                        "TERMINAL: CRM",
                        "User-Agent: PostmanRuntime/7.20.1",
                    ...
                    $custID_start = stripos($response,':')+1;
                    $acctNO_start = stripos($response,'CREATED_ACCT_NO:')+16;
                    $customer_id = substr($response, $custID_start, 10);
                    $acct_no = substr($response, $acctNO_start, 16);
                    echo "<li>Customer ID Created -: <a href='index.php?module={$bean->module_dir}&action=DetailView&record={$bean->id}'>".$customer_id."</a></li>
                        <li>Account Number Created -: $acct_no </li>";

                    $stmt = $db->query("UPDATE contacts_cstm SET client_id_c = '$customer_id' WHERE id_c = '$id_c'");
